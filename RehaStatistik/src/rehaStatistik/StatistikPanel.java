...
public class StatistikPanel extends JXPanel implements ListSelectionListener, ActionListener {
    ...
    private void holeJahr() {
        // 0 1 2 3 4 5
        String cmd = "select rez_nr,pat_intern,anzahl1,art_dbeh1,kid,preisgruppe from lza where rez_datum >='" + von
                + "' and rez_datum <= '" + bis + "' and rez_nr like 'RH%'";
        ...
        for (int i = 0; i < vec.size(); i++) {
            kuerzelID = holeKuerzelID(vec.get(i)
                                         .get(3),
                    vec.get(i)
                       .get(5),
                    vec.get(i)
            ...
            if (kuerzelID < 0) {
                ...
                rehaTage.set(kuerzelID, rehaTage.get(kuerzelID) + Integer.parseInt(vec.get(i)
                                                                                      .get(2)));
            }
        }
        if (unbekannte > 0) {
            JOptionPane.showMessageDialog(null,
                    "Unklare Rehafälle wurden entdeckt: " + Integer.toString(unbekannte) + " Fälle");
        }
    ...
    // String[] ktraegerArt =
    // {"BFA","LVA","KNP","AOK","IKK","BKK","LKK","DAK","PRI","BGE"};
    private int holeKuerzelID(String behandlung, String preisgruppe, String kassenID, List<String> kuerzel) {
        int ret = -1;

        Vector<Vector<String>> vec = SystemPreislisten.hmPreise.get("Reha")
        ...
        for (int i = 0; i < vec.size(); i++) {
            if (vec.get(i)
                ...
                if (kurz.contains("BFA")) {
                    ...
                    knamen = SqlInfo.holeFelder(
                            "select kassen_nam1,kassen_nam2 from kass_adr where id='" + kassenID + "' LIMIT 1");
    ...
    private boolean beinhaltetFragment(Vector<Vector<String>> knamen, String krit1, String krit2) {
        ...
        for (int i = 0; i < knamen.size(); i++) {
            if (knamen.get(i)
                      .get(0)
                      .toUpperCase()
                      .contains(krit1.toUpperCase())
                    || knamen.get(i)
                             .get(1)
                             .toUpperCase()
                             .contains(krit2.toUpperCase())) {
                return true;
            }
        }
        return ret;
    ...
    private void starteCalcJahr(List<String> ktraeger) {
        try {
            ...
            OOTools.doCellValue(cellCursor, 0, 1,
                    "Zeitraum: " + DatFunk.sDatInDeutsch(von) + " bis " + DatFunk.sDatInDeutsch(bis));
            ...
            if (unklareFaelle.size() > 0) {
                ...
                for (int i = 0; i < unklareFaelle.size(); i++) {
                    OOTools.doCellValue(cellCursor, 0, zeile, unklareFaelle.get(i)
                                                                           .get(0)
                            + " - " + unklareFaelle.get(i)
                                                   .get(1));
                    OOTools.doCellValue(cellCursor, 2, zeile, Double.parseDouble(unklareFaelle.get(i)
                                                                                              .get(2)));
                    zeile++;
                }
    ...
    private void holeQuartal() {
        ...
        for (int i = 0; i < vec.size(); i++) {
            ...
            if ((position = rehaArt.indexOf(kuerzel)) < 0) {
                ...
                rehaFaelle.add(1);
                rehaTage.add(Integer.parseInt(vec.get(i)
                                                 .get(2)));
            } else {
                rehaFaelle.set(position, rehaFaelle.get(position) + 1);
                rehaTage.set(position, rehaTage.get(position) + Integer.parseInt(vec.get(i)
                                                                                    .get(2)));
            }
        }
        if (rehaArt.size() > 0) {
        ...
        for (int i = 0; i < vec.size(); i++) {
            if (vec.get(i)
                ...
                if (preisgruppe.equals("3")) {
                    return "PRI-" + vec.get(i)
                                       .get(1)
                                       .toUpperCase();
                } else if (preisgruppe.equals("4")) {
                    return "BGE-" + vec.get(i)
                                       .get(1)
                                       .toUpperCase();
                } else {
                    return vec.get(i)
    ...
    private void starteCalcQuartal() {
        try {
            /*
             * rehaArt.clear(); rehaFaelle.clear(); rehaTage.clear();
    ...
    private void doWochenStatistik() {
        ...
        String cmd = "select rez_nr,pat_intern,rez_datum,termine,anzahl1,art_dbeh1,kid,preisgruppe from verordn "
                + "where ( " + "(rez_nr like 'RH%') and " + "(" + "((art_dbeh1='1' or art_dbeh1='2' or art_dbeh1='3' "
                + "or art_dbeh1='4' or art_dbeh1='6' or art_dbeh1='7') " + "and preisgruppe='6')"
        ...
        // System.out.println("Es wurden insgesamt "+Integer.toString(lang)+"
        // Reha-Verordnungen gefunden");
        int tagedifferenz = 0;
        int isttage = 0;
        int solltage = 0;
        int rehaart = 0;
        ...
        for (int i = 0; i < lang; i++) {
            ...
            try {
                isttage = tageTest(rehavec.get(i)
                                          .get(3));
            } catch (Exception ex) {
                isttage = -1;
            }
            try {
                solltage = Integer.parseInt(rehavec.get(i)
                                                   .get(4));
            } catch (Exception ex) {
                solltage = -1;
            }
            try {
                tagedifferenz = doUnklartest(rehavec.get(i)
                                                    .get(2));
            } catch (Exception ex) {
                tagedifferenz = -1;
            }
            try {
                rehaart = Integer.parseInt(rehavec.get(i)
                                                  .get(5));
            } catch (Exception ex) {
                rehaart = -1;
            }
            // System.out.println("Isttage:"+isttage+" / Solltage:"+solltage+" / Tage
            // Differenz:"+tagedifferenz+" / Rehaart:"+rehaart);
            if (isttage >= solltage) {
                berichtfehlt.add((Vector<String>) rehavec.get(i)
                                                         .clone());
                continue;
            } else if ((isttage >= 1) && (isttage < solltage)) {
                // System.out.println("in anwesend");
                doAnwesend(rehaart);
                anwesend.add((Vector<String>) rehavec.get(i)
                                                     .clone());
                continue;
            } else if ((tagedifferenz <= 14) && (isttage <= 0)) {
                // System.out.println("in angemeldet");
                doAngemeldet(rehaart);
                angemeldet.add((Vector<String>) rehavec.get(i)
                                                       .clone());
                continue;
            } else if ((tagedifferenz > 14) && (isttage <= 0)) {
                unklar.add((Vector<String>) rehavec.get(i)
                                                   .clone());
                continue;
            }
    ...
    private void starteCalcLvaWoche() {
        try {
            ...
            OOTools.doCellValue(cellCursor, 0, 13, "*********************ausstehende Entlassberichte ("
                    + berichtfehlt.size() + " Fälle) ********************");
            ...
            for (int i = 0; i < berichtfehlt.size(); i++) {
                status1.setText("ermittle fehlende Entlassberichte: " + Integer.toString(i + 1) + " von "
                        + berichtfehlt.size());
                name = SqlInfo.holePatFeld("n_name", "pat_intern='" + berichtfehlt.get(i)
                                                                                  .get(1)
                        + "'") + ", " + SqlInfo.holePatFeld("v_name",
                                "pat_intern='" + berichtfehlt.get(i)
                                                             .get(1)
                                        + "'");
                letzterRehatag = letzterRehaTag(berichtfehlt.get(i)
                                                            .get(3));
                rehaart = getRehaArt(berichtfehlt.get(i)
                                                 .get(5));
                OOTools.doCellValue(cellCursor, 0, calcrow,
                        rehaart + " - " + name + " - Entlassung am " + letzterRehatag + " Rehaverordnung: "
                                + berichtfehlt.get(i)
                                              .get(0));
            ...
            OOTools.doCellValue(cellCursor, 0, calcrow, "*********************unklare Rehafälle (" + unklar.size()
                    + " Fälle) ******************************");
            OOTools.doCellFontName(cellCursor, 0, calcrow, "Courier New");
            calcrow += 1;
            ...
            for (int i = 0; i < unklar.size(); i++) {
                ...
                name = SqlInfo.holePatFeld("n_name", "pat_intern='" + unklar.get(i)
                                                                            .get(1)
                        + "'") + ", " + SqlInfo.holePatFeld("v_name",
                                "pat_intern='" + unklar.get(i)
                                                       .get(1)
                                        + "'");
                angelegtam = DatFunk.sDatInDeutsch(unklar.get(i)
                                                         .get(2));
                rehaart = getRehaArt(unklar.get(i)
                                           .get(5));
                OOTools.doCellValue(cellCursor, 0, calcrow,
                        rehaart + " - " + name + " angelegt am: " + angelegtam + " Rehaverordnung: " + unklar.get(i)
                                                                                                             .get(0));
                OOTools.doCellFontName(cellCursor, 0, calcrow, "Courier New");
                calcrow += 1;
            }
            ...
            OOTools.doCellValue(cellCursor, 0, calcrow, "*********************anwesende Rehafälle (" + anwesend.size()
                    + " Fälle) *****************************");
            OOTools.doCellFontName(cellCursor, 0, calcrow, "Courier New");
            calcrow += 1;
            for (int i = 0; i < anwesend.size(); i++) {
                status1.setText("ermittle anwesende Rehafälle: " + Integer.toString(i + 1) + " von " + anwesend.size());
                name = SqlInfo.holePatFeld("n_name", "pat_intern='" + anwesend.get(i)
                                                                              .get(1)
                        + "'") + ", " + SqlInfo.holePatFeld("v_name",
                                "pat_intern='" + anwesend.get(i)
                                                         .get(1)
                                        + "'");
                angelegtam = DatFunk.sDatInDeutsch(anwesend.get(i)
                                                           .get(2));
                ersterRehatag = ersterRehaTag(anwesend.get(i)
                                                      .get(3));
                rehaart = getRehaArt(anwesend.get(i)
                                             .get(5));
                OOTools.doCellValue(cellCursor, 0, calcrow,
                        rehaart + " - " + name + " angelegt am: " + angelegtam + " Erster Rehatag:" + ersterRehatag
                                + " Rehaverordnung: " + anwesend.get(i)
                                                                .get(0));
                OOTools.doCellFontName(cellCursor, 0, calcrow, "Courier New");
                calcrow += 1;
            }
            ...
            OOTools.doCellValue(cellCursor, 0, calcrow, "*********************angemeldete Rehafälle ("
                    + angemeldet.size() + " Fälle) ****************************");
            OOTools.doCellFontName(cellCursor, 0, calcrow, "Courier New");
            calcrow += 1;
            for (int i = 0; i < angemeldet.size(); i++) {
                status1.setText(
                        "ermittle angemeldete Rehafälle: " + Integer.toString(i + 1) + " von " + angemeldet.size());
                name = SqlInfo.holePatFeld("n_name", "pat_intern='" + angemeldet.get(i)
                                                                                .get(1)
                        + "'") + ", " + SqlInfo.holePatFeld("v_name",
                                "pat_intern='" + angemeldet.get(i)
                                                           .get(1)
                                        + "'");
                try {
                    angelegtam = DatFunk.sDatInDeutsch(anwesend.get(i)
                                                               .get(2));
                    rehaart = getRehaArt(angemeldet.get(i)
                                                   .get(5));
                } catch (Exception ex) {
                    System.out.println("Größe des Vectors = " + anwesend.size() + " - Druchlauf = " + i);
                    System.out.println(getRehaArt(angemeldet.get(i)
                                                            .get(5)));
                    System.out.println("Name = " + name);
                    angelegtam = "nicht zu ermitteln";

                }
                OOTools.doCellValue(cellCursor, 0, calcrow, rehaart + " - " + name + " angelegt am: " + angelegtam
                        + " Rehaverordnung: " + angemeldet.get(i)
                                                          .get(0));
                OOTools.doCellFontName(cellCursor, 0, calcrow, "Courier New");
                calcrow += 1;
            }
