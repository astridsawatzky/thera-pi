...
public class SqlInfo {
    ...
    public static boolean gibtsSchon(String sstmt) {
        ...
        try {
            stmt = textbaus.thisClass.conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    /*****************************************/
    /*******************************/
    public static Vector holeSatzLimit(String tabelle, String felder, String kriterium, int[] limit,
            List ausschliessen) {
        Statement stmt = null;
        ResultSet rs = null;
        Vector<String> retvec = new Vector<String>();
        ...
        try {
            ...
            String sstmt = "select " + felder + " from " + tabelle + " " + kriterium + " LIMIT "
                    + new Integer(limit[0]).toString() + "," + new Integer(limit[1]).toString() + "";
            rs = stmt.executeQuery(sstmt);
            int nichtlesen = ausschliessen.size();
            if (rs.next()) {
                ResultSetMetaData rsMetaData = rs.getMetaData();
