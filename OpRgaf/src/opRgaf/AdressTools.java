...
public class AdressTools {
    public static String[] machePrivatAdresse(Object[] oin, boolean egross) {
        ...
        if (egross) {
            if (((String) oin[0]).toUpperCase()
                                 .equals("HERR")) {
                anrede = StringTools.EGross(anrede);
                isherr = true;
            } else if (((String) oin[0]).toUpperCase()
                                        .equals("FRAU")) {
                anrede = StringTools.EGross(anrede);
                isherr = false;
            } else {
            ...
            if (isnosex) {
                ...
                str[4] = "Sehr geehrter Herr" + (anredetitel.trim()
                                                            .length() > 0 ? " " + anredetitel.trim() : "")
                        + " " + StringTools.EGross(nname)
                                           .trim();
            } else if ((!isnosex) && (!isherr)) {
                str[4] = "Sehr geehrte Frau" + (anredetitel.trim()
                                                           .length() > 0 ? " " + anredetitel.trim() : "")
                        + " " + StringTools.EGross(nname)
                                           .trim();
            }
            ...
            str[0] = anrede;
            str[1] = String.valueOf((titel.length() > 0 ? " " + titel : "") + " " + vname + " " + nname)
                           .trim();
            str[2] = strasse;
            str[3] = (plz + " " + ort).trim();
