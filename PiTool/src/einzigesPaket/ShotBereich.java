...
public class ShotBereich extends JPanel implements MouseListener, MouseMotionListener, KeyListener, ActionListener {
    ...
    public void Zeichnen() {
        if (piTool.img != null) {
            ...
            /*
             * ToolkitImage tk = ((ToolkitImage)piTool.img.getScaledInstance(140, 100,
             * Image.SCALE_SMOOTH)); System.out.println("tk (toolkit) = "+tk); piTool.vos =
             * ((ToolkitImage)tk).getBufferedImage();
             * System.out.println("Vos = "+piTool.vos); ImageIcon img = new ImageIcon();
             * img.setImage(tk); piTool.vorschau.setIcon(img);
             */

            // Image scaled = piTool.img.getScaledInstance(140, new Integer (new Float(
            // result * 100.0000).toString()), Image.SCALE_SMOOTH );
    ...
    public void markiereBereich() {
        ...
        if (dragstart) {
            ...
            h = stoppunkt.y - y1;
            /*
             * if(altpunkt != null){ g2d.setXORMode(Color.YELLOW); g2d.setColor(
             * Color.YELLOW); g2d.drawRect(x1, y1, altpunkt.x, altpunkt.y); altpunkt.x = w;
             * altpunkt.y = h; }else{ altpunkt = new Point(stoppunkt); }
             */
    ...
    private JPopupMenu getTerminPopupMenu() {
         ...
         * men1.setRolloverEnabled(true); men1.setEnabled(true);
         * men1.setActionCommand("speichern"); men1.addActionListener(this);
         * jPopupMenu.add(men1);
         */
        return jPopupMenu;
    }
...
//		piTool.jFrame.repaint();
        ...
        /*
         * Image imgx = Image imgx = new BufferedImage(w, h,
         * BufferedImage.TYPE_INT_RGB); Graphics g = img.getGraphics();
         * //g.drawImage(imgx, w, h, null); g.drawImage(imgx, 0, 0, w, h, null);
         * g.dispose(); label.setIcon(new ImageIcon(imgx));
         * 
         */
        /*
         * System.out.println("Größe des Ausschnitts = x:"+w+" / h:"+h ); BufferedImage
         * bufi = (BufferedImage) ShotBereich.label.getIcon(); File file = new
         * File("C:\\ScreenShots\\test.jpg"); try { ImageIO.write((RenderedImage) bufi,
         * "jpg", file); } catch (IOException e) {
         * 
         * e.printStackTrace(); }
         */

        /*
         * Image img = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB); File file =
         * new File("C:\\ScreenShots\\test.jpg"); try { ImageIO.write((RenderedImage)
         * img2, "jpg", file); } catch (IOException e) {
         * 
         * e.printStackTrace(); }
         */

        /*
         * Image img = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB); Graphics g =
         * img.getGraphics(); g.drawImage(piTool.img, w, h, null); g.dispose();
         * 
         * label.setIcon(new ImageIcon(img));
         * 
         */
        /*
         * try { label.setIcon(new ImageIcon(ImageIO.read(new
         * File("C:\\ScreenShots\\test2.jpg")))); } catch (IOException e) {
         * 
         * e.printStackTrace(); }
         */
    }
    ...
    /*
     * img = (Image)t.getTransferData(DataFlavor.imageFlavor); img2 = new
     * BufferedImage(img.getWidth(jFrame), img.getHeight(jFrame),
     * BufferedImage.TYPE_INT_RGB);//ARGB für Transparenz! Graphics g =
     * img2.getGraphics(); g.drawImage(img, 0, 0, null); g.dispose();
     * 
     * orig =(Image)
     * ((ToolkitImage)((BufferedImage)t.getTransferData(DataFlavor.imageFlavor)).
     * getScaledInstance(140, 100, Image.SCALE_SMOOTH)); senden.setEnabled(true);
     */
    @Override
