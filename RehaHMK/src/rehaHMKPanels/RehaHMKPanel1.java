...
import rehaHMK.Tools.HMKTreeTableModel;
import rehaHMK.Tools.IndiKey;
...
public class RehaHMKPanel1 extends JXPanel {
    ...
    private JPanel getContent() {
        ...
        /*
         * normale Tabelle indimod.setColumnIdentifiers(new String[]
         * {"Indi.Schl�ssel","Gesamtmenge",
         * "max.pro Rezept","Vorrangiges HM","max.Vorrangig","Erg�nzend. HM"
         * ,"max.Erg�nzend."}); inditab = new JXTable(indimod);
         * inditab.setHighlighters(HighlighterFactory.createSimpleStriping(new
         * Color(204,255,255))); inditab.getSelectionModel().addListSelectionListener(
         * (indihandler=new IndiListSelectionHandler())); JScrollPane jscr =
         * JCompTools.getTransparentScrollPane(inditab); jscr.validate();
         * pan.add(jscr,cc.xyw(2,6,7)); anhaengen();
         */

        /*
         * JXTreeTable root = new JXHMKTreeTableNode("passende Indi-Schl�ssel",null,
         * true); hmkTreeTableModel = new HMKTreeTableModel(root); jXTreeTable = new
         * JXTreeTable(hmkTreeTableModel); jXTreeTable.setOpaque(true);
         * jXTreeTable.setRootVisible(true); jXTreeTable.setTreeCellRenderer(new
         * HMKTreeCellRenderer()); tsl = new HMKTreeSelectionListener();
         * jXTreeTable.addTreeSelectionListener(tsl); jXTreeTable.setSelectionMode(0);
         * JScrollPane jscr = JCompTools.getTransparentScrollPane(jXTreeTable);
         * jscr.validate(); pan.add(jscr,cc.xyw(2,6,7));
         */
        ...
        htmlScroll.getVerticalScrollBar()
                  .setUnitIncrement(15);
        htmlScroll.validate();
        ...
        hl = new HyperlinkListener() {
            ...
            public void hyperlinkUpdate(HyperlinkEvent e) {
                ...
                if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
                    ...
                    // url =
                    // "http://www.heilmittelkatalog.de/"+aktwebsite.toLowerCase()+"/"+url.toLowerCase()+".htm";
    ...
    private void fuelleCombos(int disziplin) {
        /*
         * jXTreeTable.removeTreeSelectionListener(tsl); while( root.getChildCount() >
         * 0){ hmkTreeTableModel.removeNodeFromParent((MutableTreeTableNode)
         * root.getChildAt(0)); }
         */
    ...
    private void sucheIndis() {
        /*
         * indimod.setRowCount(0); jXTreeTable.removeTreeSelectionListener(tsl); while(
         * root.getChildCount() > 0){
         * hmkTreeTableModel.removeNodeFromParent((MutableTreeTableNode)
         * root.getChildAt(0)); }
         */
        String[] suchen = { combos[0].getSecValue()
                                     .toString(),
                combos[1].getSecValue()
                         .toString() };
        String cmd = "";
        if (suchen[0].equals("")) {
        ...
        for (int i = 0; i < vec.size(); i++) {
            ...
            bufhtmlcode.append("<tr><td class='" + tdclass + "'><a class='" + aclass + "' href='http://" + vec.get(i)
                                                                                                              .get(0)
                    + "'>" + vec.get(i)
                                .get(0)
                    + "</a></td>");
            ...
            bufhtmlcode.append("<tr><td>max. pro Fall = " + vec.get(i)
                                                               .get(1)
                    + "</td></tr>");
            bufhtmlcode.append("<tr><td>max. pro Rezept = " + vec.get(i)
                                                                 .get(2)
                    + "</td></tr>");
            bufhtmlcode.append("</table>");

            Vector<String> vecx = holeHeilmittel(true, vec.get(i), vec.get(i)
                                                                      .get(1));
            if (vecx.size() > 0) {
                bufhtmlcode.append("<td class='" + tdclass + "'>");
            ...
            vecx = holeHeilmittel(false, vec.get(i), vec.get(i)
                                                        .get(1));
            if (vecx.size() > 0) {
                bufhtmlcode.append("<td class='" + tdclass + "'>");
                bufhtmlcode.append("<ul class='paeb'>");
        ...
        /*
         * for(int i = 0; i < vec.size();i++){ indimod.addRow(vec.get(i)); }
         * inditab.repaint();
         */
        /*
         * JXHMKTreeTableNode node = null; for(int i = 0; i < vec.size();i++){ IndiKey
         * key = new
    ...
    private Vector<String> holeHeilmittel(boolean vorrang, Vector<String> vec1, String maxfall) {
        ...
        if (vorrang) {
            heilmittel = vec1.get(3)
                             .split("@");
            anzahlheilmittel = vec1.get(4)
                                   .split("@");
        } else {
            heilmittel = vec1.get(5)
                             .split("@");
            anzahlheilmittel = vec1.get(6)
                                   .split("@");
        }
        String hmpos = "";
        String id = "";
        ...
        for (int i = 0; i < heilmittel.length; i++) {
            ...
            retvec.add((gleich ? "<b><u>" : "") + hmpos + " - " + getLangtextFromID(id, "", this.preisliste) + " max. "
                    + (!anzahlheilmittel[i].equals(maxfall)
                            ? "<span style='color:#FF0000;'>" + anzahlheilmittel[i] + "</span>"
                            : anzahlheilmittel[i])
                    + (gleich ? "</u></b>" : ""));
        }
        return retvec;
    ...
    private JXHMKTreeTableNode machDenRest(JXHMKTreeTableNode node, Vector<String> vec, int durchlauf) {
        ...
        IndiKey key = new IndiKey((alwayskey ? vec.get(0) : ""), "<html>" + vec.get(2) + " max. pro Rezept "
                + (nofolge.equals("") ? "" : "(<b><font color='#ff0000'>" + nofolge + "</font></b>)") + "</html>", "",
                "", durchlauf);
        JXHMKTreeTableNode xnode = new JXHMKTreeTableNode("nixgibts", key, true);
        node.insert(xnode, node.getChildCount());

        //
        String[] vorrangig = vec.get(3)
                                .split("@");
        String[] anzahlvorrangig = vec.get(4)
                                      .split("@");
        String[] ergaenzend = vec.get(5)
                                 .split("@");
        String[] anzahlergaenzend = vec.get(6)
                                       .split("@");
        ...
        for (int i = 0; i < vorrangig.length; i++) {
            ...
            id = getIDFromPos(hmpos, "", this.preisliste);
            key = new IndiKey((alwayskey ? vec.get(0) : ""), "",
                    hmpos + " - " + getLangtextFromID(id, "", this.preisliste) + " max. " + anzahlvorrangig[i], "",
                    durchlauf);
            if (i == 0) {
        ...
        for (int i = 0; i < ergaenzend.length; i++) {
            ...
            id = getIDFromPos(hmpos, "", this.preisliste);
            key = new IndiKey((alwayskey ? vec.get(0) : ""), "", "",
                    hmpos + " - " + getLangtextFromID(id, "", this.preisliste) + " max. " + anzahlergaenzend[i],
                    durchlauf);
            if (i == 0) {
    ...
    private void regleHMKSeite(int i) {
        String indi = inditab.getValueAt(i, 0)
                             .toString();
        ...
        // String url =
        // "http://www.heilmittelkatalog.de/"+aktwebsite.toLowerCase()+"/"+indi.toLowerCase()+".htm";
        // htmlPane.setPage(url);
        webBrowser.navigate(url);
    }
    ...
    class IndiTableModel extends DefaultTableModel {
        ...
        */
        ...
        public Class<?> getColumnClass(int columnIndex) {
            /*
             * if(columnIndex==0){return String.class;} if(columnIndex==1){return
             * Boolean.class;} if(columnIndex==2){return Date.class;}
             * if(columnIndex==3){return Timestamp.class;} if(columnIndex==4){return
             * String.class;} if(columnIndex==5){return String.class;}
             */
            return String.class;
        }
            ...
            if (rootNode.isLeaf()) {
                ...
                for (int i2 = 0; i2 < kindAnzahl; i2++) {
                    ...
                    if (geprueft == zeile) {
                        ...
                        childNode = (JXHMKTreeTableNode) rootNode.getChildAt(i2);
                        ////// System.out.println("Zeile �berp�ft="+geprueft+" - "+childNode.abr.datum+"
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < lang; i++) {
                                        if (aktNode == holeNode(i)) {
                                            ...
                                            // http://www.heilmittelkatalog.de/tl_files/hmk/physio/ws1.htm
                                            final String url = RehaHMK.hmkURL + aktwebsite.toLowerCase() + "/"
                                                    + indi.toLowerCase() + ".htm";
                                            // final String url =
                                            // "http://www.heilmittelkatalog.de/"+aktwebsite.toLowerCase()+"/"+indi.toLowerCase()+".htm";
                                            // htmlPane.setPage(url);
                                            // System.out.println(url);
                                            ...
                                            ////// System.out.println("Behandlungsdatum selektiert =
                                            // "+aktNode.abr.datum+" / "+aktNode.abr.bezeichnung);
                                            break;
                                        }
                                    }
                                } catch (Exception ex) {
