...
public class KassenAuswahl extends RehaSmartDialog {
    ...
    public KassenAuswahl(JXFrame owner, String name, String[] suchegleichnach, JRtaTextField[] elterntf,
            String kassennum) {
        super(owner, name);
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                ((JComponent) getContentPanel()).getActionMap()
                                                .put("doSuchen", new KassenWahlAction());
            }
        });
        SwingUtilities.invokeLater(new Runnable() {
            @Override
    ...
    public JXPanel getAuswahl() {
        ...
        kassenwahltbl.getColumn(0)
                     .setMinWidth(50);
        kassenwahltbl.getColumn(0)
                     .setMaxWidth(50);
        kassenwahltbl.getColumn(1)
                     .setMinWidth(150);
        kassenwahltbl.getColumn(2)
                     .setMinWidth(150);
        kassenwahltbl.getColumn(3)
                     .setMinWidth(100);
        kassenwahltbl.getColumn(5)
                     .setMinWidth(0);
        kassenwahltbl.getColumn(5)
                     .setMaxWidth(0);
    ...
    public void fuelleTabelle(String suchkrit) {
        ...
        if (zweikrit.length > 1) {
            krit = "(kuerzel like '%" + zweikrit[0].trim() + "%' or kassen_nam1 like '%" + zweikrit[0].trim()
                    + "%' or kassen_nam2 like '%" + zweikrit[0].trim() + "%' or ort like '%" + zweikrit[0].trim()
                    + "%' or ik_kasse like '%" + zweikrit[0].trim() + "%') AND " + "(kuerzel like '%"
                    + zweikrit[1].trim() + "%' or kassen_nam1 like '%" + zweikrit[1].trim()
                    + "%' or kassen_nam2 like '%" + zweikrit[1].trim() + "%' or ik_kasse like '%" + zweikrit[1].trim()
                    + "%' or ort like '%" + zweikrit[1].trim() + "%') order by kassen_nam1";

        } else {
            krit = "kuerzel like '%" + suchkrit + "%' or kassen_nam1 like '%" + suchkrit + "%' or kassen_nam2 like '%"
                    + suchkrit + "%' or ort like '%" + suchkrit + "%' or ik_kasse like '%" + suchkrit
                    + "%' order by kassen_nam1";
        }

        Vector<Vector<String>> vec = SqlInfo.holeSaetze("kass_adr", "kuerzel,kassen_nam1,kassen_nam2,ort,ik_kasse,id",
                krit, Arrays.asList(new String[] {}));
        int bis = vec.size();
        int i;
        for (i = 0; i < bis; i++) {
