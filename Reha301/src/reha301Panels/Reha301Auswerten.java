...
public class Reha301Auswerten extends JXPanel {
    ...
    int anzeigeart = -1;
    public String[] artderNachricht = { "Unbekannter Anlass", "Bewilligung", "Ablehnung", "Aufnahmemitteilung",
            "Unterbrechung (Beginn)", "Verlängerung", "Entlassmitteilung", "E-Bericht", "Rechnung",
            "Absage an den Kostenträger", "Einberufung", "Rückstellung", "Entlassmitteilung und Fahrgeldabrechnung",
            "Fahrgeldabrechnung", "Bestätigung der Verlängerung," + "Unterbrechung (Ende)",
            "Unterbrechung (Beginn und Ende)" };
    ...
    public JXPanel getContent() {
        ...
        tabmod.setColumnIdentifiers(new String[] { "lfnr", "import", "Sender", "Nachrichtentyp", "Name, Vorname",
                "Adresse", "VSNR", "Kostenträger", "Krankenkasse", "Datum", "", "", "", "verarbeitet" });
        ...
        tab.getSelectionModel()
           .addListSelectionListener(new MyAuswertungListSelectionHandler());
    ...
    private JScrollPane tree301() {
        ...
        for (int i = 0; i < files.length; i++) {
            if ((files[i].getName()
                         .toUpperCase()
                         .endsWith(".AUF"))
                    || (files[i].length() == 348) || (files[i].length() == 334)) {
                kobjekt = new KnotenObjekt(files[i].getName(), "", false, files[i],
                        Integer.parseInt(Long.toString(files[i].length())));
        ...
        tree.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent arg0) {
                if (arg0.getClickCount() == 2 && arg0.getButton() == 1) {
                    String treelabel = ((JXTreeNode) tree.getSelectionPath()
                                                         .getLastPathComponent()).knotenObjekt.titel;
                    int gross = ((JXTreeNode) tree.getSelectionPath()
                                                  .getLastPathComponent()).knotenObjekt.groesse;
                    if ((gross == 348) || (gross == 334)) {
                        ...
                        if (!vecgelesen.contains(treelabel)) {
                            int anfrage = JOptionPane.showConfirmDialog(null,
                                    "Wollen Sie die Nachricht " + treelabel + " einlesen und verarbeiten?",
                                    "Achtung wichtige Benutzeranfrage", JOptionPane.YES_NO_OPTION);
                            if (anfrage == JOptionPane.YES_OPTION) {
                                final String xtreelabel = treelabel;
                                final File xfile = ((JXTreeNode) tree.getSelectionPath()
                                                                     .getLastPathComponent()).knotenObjekt.file;
                                new SwingWorker<Void, Void>() {
                                    @Override
                                    protected Void doInBackground() throws Exception {
                                        try {
                                            ...
                                            if ((erfolg = einlesen.decodeAndRead(Reha301.inbox + xtreelabel, xfile))) {
                                                ...
                                                ((JXTreeNode) tree.getSelectionPath()
                                                                  .getLastPathComponent()).knotenObjekt.fertig = true;
                                            } else {
                                                //// System.out.println("Ergenis von einlesen = "+erfolg);
                                            }
                                            Reha301.thisFrame.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
                                            doNachrichtVerschieben(((JXTreeNode) tree.getSelectionPath()
                                                                                     .getLastPathComponent()).knotenObjekt.titel);
        ...
        al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (cmd.equals("rezneuanlage")) {
                    ...
                    if (!tab.getValueAt(tab.getSelectedRow(), 3)
                            .toString()
                            .contains("Ablehnung")) {
                        doRezNeuanlage();
                    } else {
    ...
    private void doNachrichtInWriter() {
        ...
        document.getTextService()
                .getText()
                .setText(SqlInfo.holeEinzelFeld(
    ...
    private Vector<Object> doSetPatientFuerNachricht(String patid) {
        ...
        if (((Integer) rVTraeger[0]) >= 0) {
             ...
             * //System.out.println("Diagnosegruppe="+diaggruppe); for(int i = 0; i <
             * rVTraeger.length;i++){ //System.out.println(rVTraeger[i]); }
             */
            isRVTraeger = true;
    ...
    private Object[] testeDTAIni(String ktraeger, String diaggruppe) {
        ...
        for (int i = 1; i <= anzahl; i++) {
            if (ini.getStringProperty("RVTraeger_" + gruppe, "RVTraegerIK" + Integer.toString(i))
                   .equals(ktraeger)) {
                retobject[0] = i;
...
//		"select eingelesen,sender,nachrichtentyp,patangaben,versicherungsnr,ktraeger,datum from dta301 where eingelesen='F' order by datum";
        ...
        try {
            ...
            if (vec.size() > 0) {
                for (int i = 0; i < vec.size(); i++) {
                    ...
                    vecobj.add(vec.get(i)
                                  .get(0)
                                  .equals("T") ? true : false);
                    vecobj.add(vec.get(i)
                                  .get(1));
                    vecobj.add(artderNachricht[Integer.parseInt(vec.get(i)
                                                                   .get(2))]
                            + "-" + vec.get(i)
                                       .get(8)
                            + "-" + vec.get(i)
                    ...
                    vecobj.add(vec.get(i)
                                  .get(5));
                    if (pat.length >= 8) {
                        kassenangaben = SqlInfo.holeEinzelFeld(
                                "select name1 from ktraeger where ikkasse='" + pat[7] + "' LIMIT 1");
                        kassenangaben = kassenangaben + String.valueOf("#" + pat[7]);
                    }
                    vecobj.add(kassenangaben);
                    try {
                        vecobj.add(DatFunk.sDatInDeutsch(vec.get(i)
                                                            .get(6)));
                    } catch (Exception ex) {
    ...
    private void show301PatData(int row) {
        ...
        buf.append("<style type='text/css'>.linksbuendig {text-align: left;}" + ".rechtsbuendig {text-align: right;}"
                + ".rbrot {text-align: right;color:#ff0000;}" + ".rbblau {text-align: right;color:#0000ff;}"
                + ".zentriert {text-align: center;}" + ".blocksatz {text-align: justify;}" + "</style>");
        buf.append("</head>");
        ...
        buf.append("<tr><td class='rbblau'>Name</td><td>" + dta301mod.getPatNachname() + ", "
                + dta301mod.getPatVorname() + "</td></tr>");
        ...
        buf.append("<tr><td class='rbblau'>Ort</td><td>" + dta301mod.getPatPlz() + " " + dta301mod.getPatOrt()
                + "</td></tr>");
    ...
    private void show301PatExist(int combovalue) {
        ...
        buf.append("<style type='text/css'>.linksbuendig {text-align: left;}" + ".rechtsbuendig {text-align: right;}"
                + ".rbrot {text-align: right;color:#ff0000;}" + ".rbblau {text-align: right;color:#0000ff;}"
                + ".rbtherapie {text-align: right;color:#e77817;}" + ".zentriert {text-align: center;}"
        ...
        buf.append("<tr><td class='rbtherapie'>Name</td><td>" + dta301mod.getX_patNachname() + ", "
                + dta301mod.getX_patVorname() + "</td></tr>");
        ...
        buf.append("<tr><td class='rbtherapie'>Ort</td><td>" + dta301mod.getX_patPlz() + " " + dta301mod.getX_patOrt()
                + "</td></tr>");
        ...
        new SocketClient().setzeRehaNachricht(Reha301.rehaPort,
                "Reha301#" + RehaIOMessages.MUST_PATFIND + "#" + dta301mod.getX_patIntern());
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    ...
                    String pat = tab.getValueAt(tab.getSelectedRow(), 4)
                                    .toString();
                    if (pat.trim()
                           .equals("")) {
                        return null;
                    }
                    String geboren = tab.getValueAt(tab.getSelectedRow(), 12)
                                        .toString();
                    //// System.out.println("Patangaben = "+pat);
                    //// System.out.println("geboren = "+geboren);
                    String cmd = "select concat(n_name,', ',v_name,', ',DATE_FORMAT(geboren,'%d.%m.%Y')) as name,anrede,n_name,v_name,geboren,strasse,plz,ort,pat_intern,kasse,id,kassenid,arzt,arztid,kv_nummer,arzt_num,telefonp from pat5 where n_name='"
                            + pat.split("#")[1] + "' and " +
                    // String cmd = "select n_name,geboren,strasse,plz,ort,pat_intern,id from pat5
                    // where n_name='"+pat.split("#")[1]+"' and "+
                    ...
                    if (vec_patstamm.size() > 0) {
                        for (int i = 0; i < vec_patstamm.size(); i++) {
                            patcombo.addItem(Integer.toString(i + 1) + " von " + Integer.toString(vec_patstamm.size())
                                    + " - " + StringTools.EGross(String.valueOf(vec_patstamm.get(i)
                                                                                            .get(0))
                                                                       .toString()));
                        }
                        patcombo.addActionListener(al);
                    ...
                    if (lsm.isSelectedIndex(i)) {
                        ...
                        if (!(Boolean) tab.getValueAt(i, 1)) {
                            ...
                            SwingUtilities.invokeLater(new Runnable() {
                                @Override
                                public void run() {
                                    Vector<Vector<String>> xvec = SqlInfo.holeFelder(
                                            "select pat_intern,rez_nr from dta301 where id = '" + dta301mod.getDtaId()
                                                    + "' LIMIT 1");
                                    if (xvec.size() > 0) {
                                        new SocketClient().setzeRehaNachricht(Reha301.rehaPort,
                                                "Reha301#" + RehaIOMessages.MUST_PATANDREZFIND + "#" + xvec.get(0)
                                                                                                           .get(0)
                                                        + "#" + xvec.get(0)
                                                                    .get(1));
                                    }
...
//Sind sind sich sicher daß keiner der rechts aufgeführten Patienten der richtige ist???
        if (patcombo.getItemCount() > 0) {
            int anfrage = JOptionPane.showConfirmDialog(null,
                    "Sind Sie sicher daß kein passender Patient in Ihrem Pat-Stamm vorhanden ist?",
                    "Achtung wichtige Benutzeranfrage", JOptionPane.YES_NO_OPTION);
            if (anfrage != JOptionPane.YES_OPTION) {
                return;
            }
        ...
        try {
            ...
            if (dta301mod.getPatKassenIk() == null || dta301mod.getPatKassenIk()
                                                               .equals("")) {
                JOptionPane.showMessageDialog(null,
                        "IK der Krankenkasse ist nicht verfügbar, Patientenanlage wird abgebrochen");
            }
            ...
            if (idkassex.trim()
                ...
                if (retwerte[2].equals("-1") || retwerte[2].equals("")) {
                    JOptionPane.showMessageDialog(null,
                            "<html><b>Achtung!!!<br><br><b>Kasse</b> konnte nicht zugeordnet werden<br><br>Patientenanlage wird abgebrochen!</html>");
                    return;
                }
                ...
                dta301mod.setPatKassenName(
                        SqlInfo.holeEinzelFeld("select kassen_nam1 from kass_adr where id='" + idkassex + "' LIMIT 1"));
            }
            // Jetzt den Arzt
            String[] retwerte = arztAuswahl(
                    new String[] { dta301mod.getPatArztName() + " " + dta301mod.getPatArztOrt(), "" });
            if (retwerte[2].equals("-1") || retwerte[2].equals("")) {
                JOptionPane.showMessageDialog(null,
                        "<html><b>Achtung!!!<br><br><b>Arzt</b> konnte nicht zugeordnet werden<br><br>Patientenanlage wird abgebrochen!</html>");
                return;
            }
            ...
            new SocketClient().setzeRehaNachricht(Reha301.rehaPort,
                    "Reha301#" + RehaIOMessages.MUST_PATFIND + "#" + patneuepatnr);
        } catch (Exception ex) {
    ...
    private void doAblehnung() {
        ...
        Vector<Vector<String>> vecreznr = SqlInfo.holeFelder(
                "select rez_nr,rez_datum from verordn where rez_nr like 'RH%' and pat_intern ='"
                        + dta301mod.getPatIntern() + "'");
        if (vecreznr.size() <= 0) {
        ...
        int anfrage = JOptionPane.showConfirmDialog(
                null, "Wollen Sie die Absage dieser Reha-Verordnung zuordnen:\n" + vecreznr.get(0)
                                                                                           .get(0)
                        + " mit Datum " + DatFunk.sDatInDeutsch(vecreznr.get(0)
                                                                        .get(1)),
                "Achtung wichtige Benutzeranfrage", JOptionPane.YES_NO_OPTION);
        if (anfrage != JOptionPane.YES_OPTION) {
            return;
        ...
        buf.append("pat_intern='" + dta301mod.getPatIntern() + "', ");
        buf.append("rez_nr='" + vecreznr.get(0)
                                        .get(0)
                + "', ");
        buf.append("nachrichtdatum='"
                + SqlInfo.holeEinzelFeld("select datum from dta301 where id ='" + id + "' LIMIT 1") + "', ");
        buf.append(
                "nachrichtorg='"
                        + StringTools.EscapedDouble(
                                SqlInfo.holeEinzelFeld("select esol from dta301 where id ='" + id + "' LIMIT 1"))
                        + "', ");
        ...
        buf.setLength(0);
        buf.trimToSize();
        buf.append("update dta301 set rez_nr='" + vecreznr.get(0)
                                                          .get(0)
                + "', ");
        buf.append("pat_intern='" + dta301mod.getPatIntern() + "', ");
        buf.append("eingelesen='" + "T" + "', ");
    ...
    private void doRezNeuanlage() {
        try {
            ...
            if (patcombo.getItemCount() > 0 && (!patneuangelegt)) {
                int anfrage = JOptionPane.showConfirmDialog(null,
                        "Wollen Sie die Verordnung auf den (rechts) ausgewählten Patienten übertragen?\nGewähle Patienten-ID = "
                                + vec_patstamm.get(patcombo.getSelectedIndex())
                                              .get(8),
                        "Achtung wichtige Benutzeranfrage", JOptionPane.YES_NO_OPTION);
                if (anfrage != JOptionPane.YES_OPTION) {
                    return;
                ...
                dta301mod.transferXPatDataTo301();
                new SocketClient().setzeRehaNachricht(Reha301.rehaPort,
                        "Reha301#" + RehaIOMessages.MUST_PATFIND + "#" + dta301mod.getPatIntern());
                ...
                JOptionPane.showMessageDialog(null,
                        "Kein bestehender Patient ausgewählt und kein neuer Patient angelegt - das geht einfach nicht!!!!");
                return;
            }
            Vector<Object> vecobj = doSetPatientFuerNachricht(welcherpat);
            StringBuffer buf = new StringBuffer();
            // nachfolgende 3 Zeilen später wieder ausschalten
            // rezneuereznr = (String) JOptionPane.showInputDialog(null, "Rezeptnummer
            // eingeben", "RH");
            // int berichtid = Integer.parseInt((String) JOptionPane.showInputDialog(null,
            // "Berichtnummer eingeben", ""));
             ...
             */
            boolean neuvo = true;
            Vector<Vector<String>> vecrhtest = SqlInfo.holeFelder("select rez_nr from verordn where pat_intern='"
                    + dta301mod.getPatIntern() + "' and rez_nr like 'RH%'");
            /******/
            ...
            if (vecrhtest.size() > 0) {
                ...
                if (frage == JOptionPane.YES_OPTION) {
                    neuvo = false;
                    rezneuereznr = String.valueOf(vecrhtest.get(0)
                                                           .get(0));
                    disziplin = "Reha";
                    rhrezexist = true;
                }
            } // Hier muß der test ob Nachsorge existiert rein. //bisheriges Zeugs ist alles
              // Murks!!! /st.
            int frage = -1;
            if (((String) vecobj.get(4)).equals("NACHSORGE")) {
                vecrhtest = SqlInfo.holeFelder("select rez_nr from verordn where pat_intern='"
                        + dta301mod.getPatIntern() + "' and (kuerzel1 like 'ASP%' or kuerzel1 like 'IRENA%')");
                if (vecrhtest.size() > 0) {
                    ...
                    msg = msg + vecrhtest.get(0)
                            ...
                            + "\n\n";
                    msg = msg + "Wollen Sie den Fall auf diese Verordnung übertragen?";
                    frage = JOptionPane.showConfirmDialog(null, msg, "Wichtige Benutzeranfrage",
                            JOptionPane.YES_NO_OPTION);
                    if (frage == JOptionPane.YES_OPTION) {
                        neuvo = false;
                        rezneuereznr = String.valueOf(vecrhtest.get(0)
            ...
            if (neuvo) {
                ...
                if (((String) vecobj.get(4)).equals("MED")) {
                    posgruppe = 2;
                    disziplin = "Reha";
                }
                if (((String) vecobj.get(4)).equals("AR")) {
                    posgruppe = 3;
                    disziplin = "Reha";
                }
                if (((String) vecobj.get(4)).equals("NACHSORGE")) {
                ...
                String preisid = RezTools.getIDFromKurzform(kuerzel, SystemPreislisten.hmPreise.get(disziplin)
                                                                                               .get(preisgruppe - 1));

                String preis = RezTools.getPreisAktFromID(preisid, SystemPreislisten.hmPreise.get(disziplin)
                                                                                             .get(preisgruppe - 1));

                String preispos = RezTools.getPosFromID(preisid, null, SystemPreislisten.hmPreise.get(disziplin)
                                                                                                 .get(preisgruppe - 1));

                String kid = SqlInfo.holeEinzelFeld(
                        "select id from kass_adr where ik_kasse ='" + ktraeger + "' LIMIT 1");
                ...
                String ktraegername = SqlInfo.holeEinzelFeld(
                        "select kassen_nam1 from kass_adr where id ='" + kid + "' LIMIT 1");
                ...
                buf.append(
                        "rez_datum='"
                                + SqlInfo.holeEinzelFeld(
                                        "select datum from dta301 where id='" + dta301mod.getDtaId() + "' LIMIT 1")
                                + "', ");
                buf.append("datum='" + DatFunk.sDatInSQL(DatFunk.sHeute()) + "', ");
                buf.append("lastdate='" + DatFunk.sDatInSQL(DatFunk.sHeute()) + "', ");
                String anzahlen = SqlInfo.holeEinzelFeld(
                        "select tage from dta301 where id='" + dta301mod.getDtaId() + "' LIMIT 1");
            ...
            buf.append("(select nachrichtentyp,id,pat_intern,rez_nr,datum,esol from dta301 where id='" + id
                    + "' LIMIT 1" + ")");
            SqlInfo.sqlAusfuehren(buf.toString());
            String fallid = SqlInfo.holeEinzelFeld("select max(id) from dtafall");
            JOptionPane.showMessageDialog(null, "<html>Die Verordnung <b>" + rezneuereznr
                    + "</b> wurde <b>erfolgreich</b> " + (neuvo ? "angelegt." : "aktualisiert.") + "</html>");
            ...
            if (!(((String) vecobj.get(4)).equals("NACHSORGE"))) {
                ...
                if (berichte > 0 && rhrezexist) {
                    ...
                    Object ret = JOptionPane.showInputDialog(null,
                            "Geben Sie bitte die Bericht-ID an auf die dieser Fall übertragen werden soll.\n(Leer lasse für neuen Bericht)",
                            "");
                    if (ret != null) {
                ...
                if (disziplin.equals("Reha")) {
                    ...
                    if (isRVTraeger) {
                        ...
                        String cmd = "insert into berhist set pat_intern='" + dta301mod.getPatIntern() + "', "
                                ...
                                + "verfasser='Reha-Arzt', " + "empfaenger='"
                                + (String) ((Object[]) ((Vector<?>) vecobj).get(0))[6] + "', "
                                + "bertitel='Reha-Entlassbericht', " + "erstelldat='"
                                + DatFunk.sDatInSQL(DatFunk.sHeute()) + "', " + "empfid='0'";
                        ...
                        buf.append("namevor='" + StringTools.Escaped(dta301mod.getPatNachname()) + ", "
                                + StringTools.Escaped(dta301mod.getPatVorname()) + "',");
                        buf.append("geboren='" + DatFunk.sDatInSQL(dta301mod.getPatGeboren()) + "',");
                    ...
                    new SocketClient().setzeRehaNachricht(Reha301.rehaPort, "Reha301#"
                            + RehaIOMessages.MUST_PATANDREZFIND + "#" + dta301mod.getPatIntern() + "#" + rezneuereznr);
    ...
    private void doNachrichtVerschieben(String datei) {
        ...
        try {
            JXTreeNode node = ((JXTreeNode) tree.getSelectionPath()
    ...
    private String[] kassenAuswahl(String[] suchenach) {
        ...
        KassenAuswahl kwahl = new KassenAuswahl(null, "KassenAuswahl", suchenach,
                new JRtaTextField[] { rettf1, rettf2, rettf3 }, "");
        kwahl.setModal(true);
        kwahl.setLocationRelativeTo(this);
        kwahl.setVisible(true);
        kwahl.dispose();
        kwahl = null;
        ret[0] = rettf1.getText()
                       .trim();
        ret[1] = rettf2.getText()
                       .trim();
        ret[2] = rettf3.getText()
                       .trim();
        return (ret);
    }
