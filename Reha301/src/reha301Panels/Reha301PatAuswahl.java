...
public class Reha301PatAuswahl extends JDialog {
    ...
    private void holePatienten() {
        ...
        String[] patdaten = nachrichtenvec.get(0)
                                          .get(22)
                                          .split("#");
        String cmd = "select concat(n_name,', ',v_name) as name,geboren,strasse,plz,ort,pat_intern,id from pat5 where n_name='"
                + StringTools.Escaped(patdaten[1]) + "' and " + "v_name='" + StringTools.Escaped(patdaten[2])
                + "' and geboren = '" + patdaten[3] + "'";
        patvec.clear();
        patvec = SqlInfo.holeFelder(cmd);
        for (int i = 0; i < patvec.size(); i++) {
            combo.addItem(StringTools.EGross(patvec.get(i)
                                                   .get(0))
    ...
    private void macheHtml(int item, Vector<Vector<String>> nachricht) {
        ...
        eilfall = pnachricht.get(0)
                            .get(24);

        String html = "<html><font face=\"Tahoma\"><br><b>" + StringTools.EGross(patvec.get(item)
                                                                                       .get(0))
                + "<br><br>" + DatFunk.sDatInDeutsch(patvec.get(item)
                                                           .get(1))
                + "<br><br>" + StringTools.EGross(patvec.get(item)
                                                        .get(2))
                + "<br><br>" + StringTools.EGross(patvec.get(item)
                                                        .get(3))
                + " " + StringTools.EGross(patvec.get(item)
                                                 .get(4))
                + "<br><br>" + verordnungsart + "<br>" + eilfall + "</b></font></html>";
