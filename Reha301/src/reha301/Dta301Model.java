...
public class Dta301Model {
    ...
    public void set301Data() {
        try {
            String pat = table.getValueAt(table.getSelectedRow(), 4)
                              .toString();
            String adress = table.getValueAt(table.getSelectedRow(), 5)
                                 .toString();
            String vsnr = table.getValueAt(table.getSelectedRow(), 6)
                               .toString();
            String ktraeger = table.getValueAt(table.getSelectedRow(), 7)
                                   .toString();
            String anlass = table.getValueAt(table.getSelectedRow(), 3)
                                 .toString();
            String kkasse = table.getValueAt(table.getSelectedRow(), 8)
                                 .toString();
            String geboren = DatFunk.sDatInDeutsch(table.getValueAt(table.getSelectedRow(), 12)
                                                        .toString());
            ...
            try {
                this.patArztName = SqlInfo.holeEinzelFeld(
                        "select adr1 from dta301 where id='" + this.getDtaID() + "' LIMIT 1")
                                          .split("#")[0];
            } catch (Exception ex) {
                this.patArztName = "";
            }
            ...
            this.patArztId = null;
            this.patTelefon = SqlInfo.holeEinzelFeld(
                    "select adr0 from dta301 where id='" + this.getDtaID() + "' LIMIT 1");
            try {
                this.patArztOrt = SqlInfo.holeEinzelFeld(
                        "select adr1 from dta301 where id='" + this.getDtaID() + "' LIMIT 1")
                                         .split("#")[1];
            } catch (Exception ex) {
                this.patArztOrt = "";
