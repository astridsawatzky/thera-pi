...
public class TheraPiUpdates implements WindowListener {
    ...
    public static void main(String[] args) {
        ...
        /*
         * Properties systemproperties = System.getProperties(); for (Enumeration e =
         * systemproperties.propertyNames(); e.hasMoreElements(); ) { String prop =
         * (String) e.nextElement(); System.out.println("Property: " + prop +
         * " , Wert: " + systemproperties.getProperty(prop)); }
         */

        try {
            devcomputer = java.net.InetAddress.getLocalHost()
                                              .toString();
    ...
    public JFrame createJFrame() {
        ...
        jFrame.setSize(ssize.width * 3 / 4 >= 800 ? ssize.width * 3 / 4 : 800,
                ssize.height / 2 >= 600 ? ssize.height / 2 : 600);
        jFrame.setPreferredSize(new Dimension(ssize.width * 3 / 4 >= 800 ? ssize.width * 3 / 4 : 800,
                ssize.height / 2 >= 600 ? ssize.height / 2 : 600));
        ...
        try {
             ...
             * p.getOutputStream().close(); // close Process' stdin BufferedReader r = new
             * BufferedReader( new InputStreamReader(p.getInputStream()) );
             * 
             * String strLine = ""; while( (strLine = r.readLine()) != null ) {
             * //System.out.println(strLine); if(
             * strLine.trim().startsWith("Physikalische Adresse") ) { strMACAdr =
             * strLine.trim().substring( 36 ); System.out.println(strMACAdr); break; } }
             */
         ...
         * dec.init(Verschluesseln.getPassword().toCharArray(), dec.getSalt(),
         * dec.getIterations());
         */
        ...
        if (!f.exists()) {
            ...
            try {
                ...
                /*
                 * this.macAdr = dec.decrypt(inif.getStringProperty("Updates", "1")); this.email
                 * = dec.decrypt(inif.getStringProperty("Updates", "2")); this.pw =
                 * dec.decrypt(inif.getStringProperty("Updates", "3"));
                 * System.out.println(macAdr+" / "+email+" / "+pw);
                 */
                /*
                 * BufferedReader in = new BufferedReader(new
                 * FileReader(UpdateConfig.getProghome()+"Libraries/lib/ocf/sig.jar"));
                 * 
                 * this.macAdr = dec.decrypt(in.readLine().trim());
                 * //System.out.println(macAdr); //System.out.println(strMACAdr); this.email =
                 * dec.decrypt(in.readLine().trim()); //System.out.println(email); this.pw =
                 * dec.decrypt(in.readLine().trim());
                 * //System.out.println(dec.decrypt(pw.trim().replace(System.getProperty(
                 * "line.separator"),""))); System.out.println(macAdr+" / "+email+" / "+pw);
                 * in.close();
                 */
        ...
        if (UpdateConfig.getInstance()
            ...
            jFrame.getContentPane()
                  .add(updateTab = new UpdateTab(this, jFrame));
        } else {
            jFrame.getContentPane()
                  .add(updatePanel = new UpdatePanel(this, jFrame, null));
        }
        jFrame.pack();
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (macNotMatch) {
                    try {
                        ...
                        File fi = new File(PROGHOME + "ini/tpupdateneu.ini");
                        if (fi.exists()) {
                            // System.out.println("Vor der Verschlüsselung Datei existiert =
                            // "+fi.getName());
                            UpdateConfig.isrta = false;
                        } else {
                         ...
                         * dec.getIterations());
                         * 
                         * String decmac = dec.encrypt(strMACAdr.trim()); FileWriter f = new FileWriter
                         * (UpdateConfig.getProghome()+"Libraries/lib/ocf/sig.jar"); f.flush();
                         * f.close(); INIFile inif = new
                         * INIFile(UpdateConfig.getProghome()+"Libraries/lib/ocf/sig.jar");
                         * inif.setStringProperty("Updates", "1",decmac ,null);
                         * inif.setStringProperty("Updates", "2",dec.encrypt(field1.getText()) ,null);
                         * inif.setStringProperty("Updates", "3",dec.encrypt(pw) ,null); inif.save();
                         */
                        /*
                        ...
                        /*
                         * f.write(decmac); f.write(System.getProperty("line.separator"));
                         * f.write(dec.encrypt(field1.getText()));
                         * f.write(System.getProperty("line.separator")); f.write(dec.encrypt(pw));
                         * f.write(System.getProperty("line.separator")); f.flush(); f.close();
                         */
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
