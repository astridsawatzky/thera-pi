 ...
 //   private int aktSet;
    ...
    public JXPanel init(int setOben, Ansicht ansicht, JRehaInternal eltern, Connection connection) {
        ...
        if (this.aktAnsicht == Ansicht.MASKE) {
            ...
            macheStatement(sstmt, aktAnsicht == Ansicht.NORMAL ? KollegenLaden.maxKalZeile : 7);
        }
    ...
    private void comboListenerInit(final int welche, Connection connection) {
        ...
        oCombo[welche].addActionListener(new java.awt.event.ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent e) {
                int wahl = KollegenLaden.suchen((String) oCombo[welche].getSelectedItem());
                if (!oCombo[welche].isPopupVisible()) {
                    oCombo[welche].setPopupVisible(false);
                }
                if (TerminFenster.this.aktAnsicht == Ansicht.NORMAL) {
                    try {
                        belegung[welche] = KollegenLaden.vKKollegen.get(wahl).Reihe - 1;
                        oSpalten[welche].datenZeichnen(vTerm, belegung[welche]);
                        oSpalten[aktiveSpalte[2]].requestFocus();
                        if (welche == 0) {
                            wochenbelegung = KollegenLaden.vKKollegen.get(wahl).Reihe;
                        }
                    ...
                    maskenbelegung = KollegenLaden.vKKollegen.get(wahl).Reihe;
    ...
    public void setCombosOutside() {
        ...
        for (von = 0; von < bis; von++) {
            oCombo[0].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
            oCombo[1].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
            oCombo[2].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
            oCombo[3].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
            oCombo[4].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
            oCombo[5].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
            oCombo[6].addItem(KollegenLaden.vKKollegen.get(von).Matchcode);
        }
    ...
    private String ansichtStatement(String stag, Ansicht ansicht) {
        ...
        if (aktAnsicht == Ansicht.NORMAL) {
            ...
            macheStatement(sstate, aktAnsicht == Ansicht.NORMAL ? KollegenLaden.maxKalZeile : 7);
            /* bislang aktiv */
            ...
            if (behandler == 0) {
                behandler = KollegenLaden.vKKollegen.get(
                        KollegenLaden.suchen((String) oCombo[0].getSelectedItem())).Reihe;
            }
    ...
    private void maskenStatement(String sstmt) {
        ...
        try {
            ...
            try {
                ...
                int maxbehandler;
                if (aktAnsicht == Ansicht.NORMAL) {
                    maxbehandler = KollegenLaden.vKKollegen.size();
                } else {
                    maxbehandler = 7;
                }
                int maxblock = 0;
    ...
    private void testeObAusmustern() {
        ...
        if (aktAnsicht == Ansicht.NORMAL) {
            ...
            xBehandler = KollegenLaden.getKollegenUeberReihe(xaktBehandler + 1);
        } else if (aktAnsicht == Ansicht.WOCHE) {
            xaktBehandler = Integer.parseInt(Integer.toString(aktiveSpalte[2]));
            xBehandler = KollegenLaden.getKollegenUeberReihe(wocheBehandler);
        }
    ...
    void terminAufnehmen(int behandler, int block) {
        ...
        try {
            ...
            if (aktAnsicht == Ansicht.NORMAL) {
                sTerminVergabe[5] = KollegenLaden.getKollegenUeberReihe(xaktBehandler + 1);
                sTerminVergabe[6] = Integer.toString(behandler + 1);
            } else if (aktAnsicht == Ansicht.WOCHE) {
                sTerminVergabe[5] = KollegenLaden.getKollegenUeberReihe(wocheBehandler);
    ...
    public void setzeTerminAktuell(String adatum, String auhrzeit, String abehandler) {
        if (aktAnsicht == Ansicht.NORMAL) {
            ...
            int azeile = KollegenLaden.getDBZeile(oCombo[setspalte].getSelectedIndex()) - 1;
            int ablock = -1;
                            ...
                            try {
                                ...
                                if (!grobRaus) {
                                    ...
                                    if (altaktiveSpalte[2] == spAktiv) {
                                        ...
                                        for (int i2 = 0; i2 < lang; i2++) {
                                            if (((Vector<?>) ((ArrayList<?>) vTerm.get(behandler)).get(2)).get(i2)
                                                ...
                                                try {
                                                    ...
                                                    String altbehandler = "";
                                                    if (aktAnsicht == Ansicht.NORMAL) {
                                                        altbehandler = KollegenLaden.getKollegenUeberReihe(
                                                                behandler + 1);
                                                    } else if (aktAnsicht == Ansicht.WOCHE) {
                                                        altbehandler = KollegenLaden.getKollegenUeberReihe(
                                                                wocheBehandler);
                                        ...
                                        String altbehandler = "";
                                        if (aktAnsicht == Ansicht.NORMAL) {
                                            altbehandler = KollegenLaden.getKollegenUeberReihe(ialtbehandler + 1);
                                        } else if (aktAnsicht == Ansicht.WOCHE) {
                                            altbehandler = KollegenLaden.getKollegenUeberReihe(wocheBehandler);
                                        }
                ...
                try {
                    ...
                    if (vec.size() > 0) {
                        ...
                        Object[] objTerm = RezTools.BehandlungenAnalysieren(swreznum, false, xforceDlg, false,
                                ...
                                KollegenLaden.getKollegenUeberDBZeile(swbehandler + 1),
