...
public class BarCodeScanner implements Runnable, SerialPortEventListener {
    ...
    public BarCodeScanner(String port) throws Exception {
        ...
        try {
            ...
            serialPort.setSerialPortParams(baud, bits, stopbit, parity); //
            serialPort.setFlowControlMode(SerialPort.FLOWCONTROL_NONE); //

        } catch (PortInUseException e) {
            // System.out.println("Port für Barcode-Scanner "+port+" konnte nicht geöffnet
            // werden.\n\nBereits belegt?");
            // System.out.println("Derzeitiger Besitzer = "+e.currentOwner);
            return;
            // JOptionPane.showMessageDialog(null, "Port f�r Barcode-Scanner "+port+" konnte
    ...
    public void run() {
        /*
         * while(Reha.instance != null){ try { Thread.sleep(2000); } catch
    ...
    public void serialEvent(SerialPortEvent event) {
        ...
        case SerialPortEvent.DATA_AVAILABLE:
            ...
            try {
                int numBytes = 0;
                /*
                 * while (inputStream.available() > 0 ) { numBytes = inputStream.read();
                 * if(numBytes >= 32){ zeichen[0] = (char) numBytes; sb.append(zeichen);
                 * 
                 * } }
                 */
                ...
                outString = sb.append(byteArrayOutputStream.toString())
                              .toString();
                ...
                if (outString.length() >= 2) {
                    if ("KGMAERLORHPORSFT".contains(outString.substring(0, 2))) {
                        ...
                        erfassen.start();
                    } else if (outString.substring(0, 1)
                                        .equals("S")) {
