...
//			String dummy2 = dummy1.split("-")[0];
			...
			int behTage = jXTreeTable.getRowCount();			// 16160: 6 (ok) -> hmap
			rezeptSichtbar = true;
		/*	
		}catch(Exception ex){
			ex.printStackTrace();
	...
	private void regleAbrechnungsModus(){
		...
		if(tog.isSelected()){
			...
			if(this.jXTreeTable.getRowCount() > 0){
				...
				while(vec_rez_valid == false){		// sucheRezept() ist noch nicht fertig...
					try {
						Thread.sleep(25);
					} catch (InterruptedException ex) {
						ex.printStackTrace();
...
//				boolean zuzahlung,double rezgeb,String unterbrechung,String alterpreis,String sqldatum,String preisid){
	...
	private boolean holeEDIFACT(String rez_nr){
		...
		*/
		...
		vec_rez_valid = false;			//ungültig bis neu belegt
		new SwingWorker<Void,Void>(){
			@Override
			protected Void doInBackground() throws Exception {
				inworker = true;
