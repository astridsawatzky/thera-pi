    ...
    public static String[] getLeistung(String hmart, String hmposition, int preisgruppe) {
        ...
        for (int i = 0; i < 1;) {
            if (hmart.equals("KG")) {
                ...
                vec = SystemPreislisten.hmPreise.get("Physio")
                                                .get(preisgruppe);
                break;
            }
            if (hmart.equals("MA")) {
                // vec = ParameterLaden.vMAPreise;
                vec = SystemPreislisten.hmPreise.get("Massage")
                                                .get(preisgruppe);
                break;
            }
            if (hmart.equals("ER")) {
                // vec = ParameterLaden.vERPreise;
                vec = SystemPreislisten.hmPreise.get("Ergo")
                                                .get(preisgruppe);
                break;
            }
            if (hmart.equals("LO")) {
                // vec = ParameterLaden.vLOPreise;
                vec = SystemPreislisten.hmPreise.get("Logo")
                                                .get(preisgruppe);
                break;
            }
            if (hmart.equals("RH")) {
                // vec = ParameterLaden.vRHPreise;
                vec = SystemPreislisten.hmPreise.get("Reha")
                                                .get(preisgruppe);
                break;
            }
            if (hmart.equals("PO")) {
                // vec = ParameterLaden.vRHPreise;
                vec = SystemPreislisten.hmPreise.get("Podo")
                                                .get(preisgruppe);
                break;
            }
        ...
        for (int i = 0; i < lang; i++) {
            if (Integer.parseInt(vec.get(i)
                                    .get(9)) == ipos) {
                return new String[] { String.valueOf(vec.get(i)
                                                        .get(0)),
                        String.valueOf(vec.get(i)
                                          .get(1)) };
