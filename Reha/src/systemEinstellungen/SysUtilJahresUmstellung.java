...
public class SysUtilJahresUmstellung extends JXPanel implements KeyListener, ActionListener {
    ...
    /*
     * JLabel preisgruppen = null; JLabel infolab = null;
     */
    ...
    SysUtilJahresUmstellung() {
        ...
        /****/
        /*
         * JScrollPane jscr = new JScrollPane(); jscr.setBorder(null);
         * jscr.setOpaque(false); jscr.getViewport().setOpaque(false);
         * jscr.setViewportView(getUmstellungsSeite());
         * jscr.getVerticalScrollBar().setUnitIncrement(15); jscr.validate();
         * 
         * add(jscr);
         */
    ...
    private JPanel getUmstellungsSeite() {
        ...
        letztesjahr = new JLabel(Integer.toString(
                Integer.parseInt(SqlInfo.holeEinzelFeld("select altesjahr from jahresabschluss")) + 1));
        ...
        letztesdatum = new JLabel(
                DatFunk.sDatInDeutsch(SqlInfo.holeEinzelFeld("select umgestellt from jahresabschluss")));
        ...
        /*
         * preisgruppen = new JLabel(""); builder.add(preisgruppen,
         * cc.xy(2,19,CellConstraints.FILL,CellConstraints.DEFAULT)); buts[2] =
         * ButtonTools.macheBut("preisgr.", "tarifemachen", this); builder.add(buts[2],
         * cc.xy(4,19,CellConstraints.FILL,CellConstraints.DEFAULT)); infolab = new
         * JLabel(""); builder.add(infolab,
         * cc.xy(2,21,CellConstraints.FILL,CellConstraints.DEFAULT));
         */
    ...
    public void actionPerformed(ActionEvent arg0) {
                ...
                protected Void doInBackground() throws Exception {
                    try {
                        int frage = JOptionPane.showConfirmDialog(null,
                                "<html>Sie stellen jetzt die Patienten- und Rezeptdaten für das <b>Geschäftsjahr <font color='#ff0000'>"
                                        + nextyear.getText()
                                        + "</font></b> (unumkehrbar) um.<br>Wollen Sie die Umsetzung jetzt starten</html>",
                        ...
                        JOptionPane.showMessageDialog(null,
                                "Daten wurden erfolgreich umgesetzt - wir wünschen Ihnen gute Geschäfte im neuen Kalenderjahr!");
                    } catch (Exception ex) {
        ...
        /*
         * if(cmd.equals("tarifemachen")){ new SwingWorker<Void,Void>(){
         * 
         * @Override protected Void doInBackground() throws Exception {
         * doTarifeErzeugen(); return null; } }.execute(); }
         */
    ...
    public void doUmstellen() {
        ...
        String cmd = "select t2.id from verordn as t2 inner join pat5 as t1 on (t2.pat_intern = t1.pat_intern) where t2.befr='T' and t2.zzstatus='0' and (not t1.jahrfrei like '"
                + letztesjahr.getText() + "%')";
        vec = SqlInfo.holeFelder(cmd);
        if (vec.size() != umstellrezept) {
            JOptionPane.showMessageDialog(null,
                    "Die gestesteten Werte und die jetzt ermittelten Werte stimmen nicht überein!\nUmstellung wird nicht durchgeführt");
            return;
        }
        ...
        try {
            writer = new BufferedWriter(
                    new FileWriter(new File(Path.Instance.getProghome() + "umstellung" + reset + ".txt")));
        ...
        for (int i = 0; i < umstellrezept; i++) {
            ...
            try {
                anzahl = RezTools.holeEinzelTermineAusRezept("",
                        SqlInfo.holeEinzelFeld("select termine from verordn where id='" + vec.get(i)
                                                                                             .get(0)
                                + "' LIMIT 1"))
                                 .size();
                reznr = SqlInfo.holeEinzelFeld("select rez_nr from verordn where id='" + vec.get(i)
                                                                                            .get(0)
                        + "' LIMIT 1");
                patintern = SqlInfo.holeEinzelFeld("select pat_intern from verordn where id='" + vec.get(i)
                                                                                                    .get(0)
                        + "' LIMIT 1");
                patname = SqlInfo.holeEinzelFeld(
                        "select concat(n_name,', ',v_name) as name from pat5 where pat_intern='" + patintern
                                + "' LIMIT 1");
                int aktbefreit = -1;
                try {
                    aktbefreit = Integer.parseInt(SqlInfo
                                                         .holeEinzelFeld("select bef_dat from pat5 where pat_intern='"
                                                                 + patintern + "' LIMIT 1")
                                                         .substring(0, 4));
                ...
                if (anzahl >= Integer.parseInt(
                        SqlInfo.holeEinzelFeld("select anzahl1 from verordn where id='" + vec.get(i)
                                                                                             .get(0)
                                + "' LIMIT 1"))) {
                    // Wenn das Rezept bereits voll ist.....
                    meldung = "Das Rezept ->" + reznr + " hat bereits " + Integer.toString(anzahl)
                            + " Termine und ist damit voll.\n" + "Patient: " + patname + "\n\n"
                            + "Den Befreiungsstatus für dieses Rezept auf BEFREIT belassen?\n";
                    anfrage = JOptionPane.showConfirmDialog(null, meldung, "Achtung wichtige Benutzeranfrage",
                            JOptionPane.YES_NO_OPTION);
                    if (anfrage == JOptionPane.YES_OPTION) {
                        cmd = "update verordn set jahrfrei='" + reset + "' where id='" + vec.get(i)
                                                                                            .get(0)
                                + "' LIMIT 1";
                        ...
                        cmd = "update verordn set befr='F',rez_geb='0.00',rez_bez='F',zzstatus='2',jahrfrei='" + reset
                                + "' where id='" + vec.get(i)
                                                      .get(0)
                                + "' LIMIT 1";
                        SqlInfo.sqlAusfuehren(cmd);
                    ...
                    if (aktbefreit <= Integer.parseInt(reset)) {
                        ...
                        meldung = "Der Patient: " + patname + " hat bereits eine Befreiung für das Jahr "
                                + Integer.toString(aktbefreit) + " eingetragen!\n" + "Soll das Rezept --> " + reznr
                                + " auf Status BEFREIT belassen werden?\n";
                        anfrage = JOptionPane.showConfirmDialog(null, meldung, "Achtung wichtige Benutzeranfrage",
                                JOptionPane.YES_NO_OPTION);
                        if (anfrage == JOptionPane.YES_OPTION) {
                            cmd = "update verordn set jahrfrei='" + reset + "' where id='" + vec.get(i)
                                    ...
                                    + "' LIMIT 1";
                            SqlInfo.sqlAusfuehren(cmd);
                            writer.write("!!!!!!!!!Rezept nicht!!!! umgesetzt --> " + reznr + " - Patient: " + patname
                                    + " ist bereits für " + Integer.toString(aktbefreit) + " befreit");
                            writer.newLine();
                        } else {
                            cmd = "update verordn set befr='F',rez_geb='0.00',rez_bez='F',zzstatus='2',jahrfrei='"
                                    + reset + "' where id='" + vec.get(i)
                                                                  .get(0)
                                    + "' LIMIT 1";
                            SqlInfo.sqlAusfuehren(cmd);
                            writer.write("Rezept regulär umgesetzt --> " + reznr + " - Patient: " + patname
                ...
                JOptionPane.showMessageDialog(null,
                        "Achtung es ist ein Fehler aufgetreten bei der Umstellung des Rezeptes mit id = " + vec.get(i)
                                                                                                               .get(0));
                try {
                    writer.newLine();
                    writer.write("!!!!!!!!!Fehler bei Rezept --> " + reznr + " - Patient: " + patname
        ...
        try {
            ...
            cmd = "update pat5 set befreit='F',bef_ab=null,bef_dat=null,jahrfrei='" + reset
                    + "' where befreit = 'T' and bef_dat like '" + reset + "%'";
            SqlInfo.sqlAusfuehren(cmd);
            writer.newLine();
            writer.write(cmd);
            writer.newLine();
            ...
            cmd = "update jahresabschluss set altesjahr='" + reset + "',umgestellt='"
    ...
    public void doTesten() {
        ...
        String cmd = "select count(*) from pat5 where befreit = 'T' and (bef_dat like '" + letztesjahr.getText()
                + "%')";
        vec = SqlInfo.holeFelder(cmd);
        umstellpatient = Integer.parseInt(vec.get(0)
                                             .get(0));

        String meldung = "Patienten die zurückgesetzt werden müssen: " + Integer.toString(umstellpatient) + "\n";

        cmd = "select count(*) from verordn as t2 inner join pat5 as t1 on (t2.pat_intern = t1.pat_intern) where t2.befr='T' and t2.zzstatus='0' and (not t1.jahrfrei like '"
                + letztesjahr.getText() + "%')";
        vec = SqlInfo.holeFelder(cmd);
        umstellrezept = Integer.parseInt(vec.get(0)
                                            .get(0));
    ...
    /*
     * private void doTarifeErzeugen(){ Vector<Vector<String>> kass_vec = null; int
     * anzahlinrez=0,anzahlinlza=0; int atmen = 0; String cmd = ""; for(int i = 7; i
     * < 11; i++){ kass_vec =
     * SqlInfo.holeFelder("select kassen_nam1,id from kass_adr where preisgruppe='"
     * +Integer.toString(i)+"'"); for(int y = 0; y < kass_vec.size();y++){
     * infolab.setText("PG="+Integer.toString(i)+" "+kass_vec.get(y).get(0)+" "+
     * Integer.toString(y+1)+" von "+Integer.toString(kass_vec.size())); anzahlinrez
     * = Integer.parseInt(SqlInfo.
     * holeEinzelFeld("select count(*) from verordn where kid ='"+kass_vec.get(y).
     * get(1)+"'")); if(anzahlinrez > 0){ cmd = "update verordn set preisgruppe='"+
     * Integer.toString(i)+"', ktraeger='"+kass_vec.get(y).get(0)+
     * "' where kid='"+kass_vec.get(y).get(1)+"'";
     * preisgruppen.setText("R-"+kass_vec.get(y).get(0)+"mit "+Integer.toString(
     * anzahlinrez)+" Rezepten"); SqlInfo.sqlAusfuehren(cmd);
     * //System.out.println(cmd); } anzahlinlza = Integer.parseInt(SqlInfo.
     * holeEinzelFeld("select count(*) from lza where kid ='"+kass_vec.get(y).get(1)
     * +"'")); if(anzahlinlza > 0){ cmd = "update lza set preisgruppe='"+
     * Integer.toString(i)+"', ktraeger='"+kass_vec.get(y).get(0)+
     * "' where kid='"+kass_vec.get(y).get(1)+"'";
     * preisgruppen.setText("L-"+kass_vec.get(y).get(0)+" mit "+Integer.toString(
     * anzahlinlza)+" Rezepten"); SqlInfo.sqlAusfuehren(cmd);
     * //System.out.println(cmd); }
     * 
     * atmen++; if(atmen > 25){ try { Thread.sleep(50); atmen = 0; } catch
