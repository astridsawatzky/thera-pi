...
public class SysUtilOpMahnung extends JXPanel implements KeyListener, ActionListener {
    ...
    public SysUtilOpMahnung(ImageIcon img) {
         ...
         * jscr.validate(); jscr.getVerticalScrollBar().setUnitIncrement(15);
     ...
     * 
     * tfs[0].setText(SystemConfig.hmZusatzInOffenPostenIni.get("RGRinOPverwaltung")
     * .toString());
     * tfs[1].setText(SystemConfig.hmZusatzInOffenPostenIni.get("AFRinOPverwaltung")
     * .toString());
     * 
     * // Merken der Original eingelesenen Textfelder SaveChangeStatus(); }
     */
    /*
     * //String cmd =
     * "select * from nummern where mandant = '"+Reha.aktIK+"' LIMIT 1"; String cmd
     * = "select * from nummern LIMIT 1"; Vector<Vector<String>> vec =
     * SqlInfo.holeFelder(cmd); if(vec.size() <= 0){ JOptionPane.showMessageDialog(
     * null,"Achtung für den aktuellen Mandanten existiert noch kein Nummernkreis");
     * nummernkreisok = false; return; }
     */
    ...
    private JPanel getVorlagenSeite() {
        ...
        FormLayout lay = new FormLayout(
                "right:max(60dlu;p), 4dlu, 40dlu, 4dlu, 40dlu, 4dlu, 40dlu, 4dlu, 40dlu, 4dlu, 40dlu",
        ...
        JLabel lab = new JLabel(
                "<html><b>Rechnungsgebühren- & Ausfall-Rechnung unter Offene Posten und Mahnung handhaben</b></html>");
...
//		vec.addElement("0");
        ...
        // vec_kuerzel.add(1, (Vector<String>)(Arrays.asList( new String[]
        // {"Nein","0"})));
...
//		String strTest2 = (String)cmbRGR.getSelectedItem();
        cmbRGR.setSelectedItem(tfs[0].getText()
                                     .equals("0") ? "Nein" : "Ja"); // setze den aktuell gewählten Wert
...
//		vec_kuerzel.add(1,(Vector<String>) vec.clone());
        ...
        cmbAFR.setSelectedItem(tfs[1].getText()
                                     .equals("0") ? "Nein" : "Ja"); // setze den aktuell gewählten Wert
...
//		builder.addLabel("RGR u. AFR in OPs testen", cc.xy(1, iAktY ));
        ...
        /*
         * 
         * iAktY += 2; JLabel lab3 = new
         * JLabel("<html><b>Werkzeug-Dialoge <font color=#0000FF>(Benutzer individulle Einstellung)</font></b></html>"
         * ); builder.add(lab3,cc.xyw(1, iAktY, 11));
         * 
         * iAktY += 2;
         * builder.addLabel("Zusatzknopf \"ausführen\" anzeigen",cc.xy(1,iAktY)); //
         * builder.addLabel("(0, 1)",cc.xy(5,iAktY)); tfs[3] = new
         * JRtaTextField("Zahlen", true); tfs[3].setName("zusatzknopf");
         * tfs[3].setText((Boolean)SystemConfig.hmPatientenWerkzeugDlgIni.get(
         * "ToolsDlgShowButton") ? "1" : "0"); // builder.add(tfs[3],cc.xy(3,iAktY)); //
         * ComboBox vorbereiten // vec.clear(); // für die nächste Belegung freimachen
         * // Collections.addAll( vec, "Ja", "1" ); //
         * vec_kuerzel.add(0,(Vector<String>) vec.clone()); // vec.clear(); // für die
         * nächste Belegung freimachen // Collections.addAll( vec, "Nein", "0" ); //
         * vec_kuerzel.add(1,(Vector<String>) vec.clone());
         * cmbBut.setDataVectorVector(vecJN, 0, 1);
         * cmbBut.setSelectedItem(tfs[3].getText().equals("0") ? "Nein" : "Ja" ); //
         * setze den aktuell gewählten Wert cmbBut.setActionCommand("cmbBut");
         * cmbBut.addActionListener(this); builder.add(cmbBut, cc.xy(3,iAktY));
         * 
         * iAktY += 2;
         * builder.addLabel("Anzahl Mausklicks zum Werkzeugstart",cc.xy(1,iAktY)); //
         * builder.addLabel("(1, 2)",cc.xy(5,iAktY)); tfs[2] = new
         * JRtaTextField("ZAHLEN", true); tfs[2].setName("anzmaus");
         * tfs[2].setText(SystemConfig.hmPatientenWerkzeugDlgIni.get(
         * "ToolsDlgClickCount").toString()); // builder.add(tfs[2],cc.xy(3,iAktY)); //
         * ComboBox vorbereiten vec12.clear(); vec.clear(); // für die nächste Belegung
         * freimachen Collections.addAll( vec, "1 Klick", "1" );
         * vec12.add(0,(Vector<String>) vec.clone()); vec.clear(); // für die nächste
         * Belegung freimachen Collections.addAll( vec, "2 Klicks", "2" );
         * vec12.add(1,(Vector<String>) vec.clone()); cmbClk.setDataVectorVector(vec12,
         * 0, 1); cmbClk.setSelectedItem(tfs[2].getText().equals("1") ? "1 Klick" :
         * "2 Klicks" ); // setze den aktuell gewählten Wert
         * cmbClk.setActionCommand("cmbClk"); cmbClk.addActionListener(this);
...
//			System.out.println("cmbAFR.getSecValue() = " + cmbAFR.getSecValue());
    ...
    // prüft, wieviele RGR und AFR breits in rliste drin sind und wieviele in
    // rgaffakture enthalten sind
    private void doTesten() {
        ...
        JOptionPane.showMessageDialog(null, "Zur OP & Mahnungshadhabung sind bereits übertragen: " + strCntRgrRliste
                + " RGR und " + strCntAfrRliste + " AFR\n" + "Fakturiert sind insgesamt: " + strCntRgrFaktura
                + " RGR und " + strCntAfrFaktura + " AFR. "
...
//		int iSize2 = vec2.size();
        ...
        for (int z = 0; z < vec2.size(); z++) { // Datensatz für Datensatz
            ...
            cmd = "select x_nummer from rliste where x_nummer='" + vec2.get(z)
                                                                       .get(0)
                    + "'";
...
//			strHelp = vec.get(0).get(0);
            ...
            if (vec.size() <= 0 // nur wenn es den Datensatz in rliste noch NICHT gibt
                    && ((vec2.get(z)
                             .get(0)
                             .contains("RGR-")
                            && SystemConfig.hmZusatzInOffenPostenIni.get("RGRinOPverwaltung") == 1) // RGR zugelassen ?
                            || (vec2.get(z)
                                    .get(0)
                                    .contains("AFR-")
                                    && SystemConfig.hmZusatzInOffenPostenIni.get("AFRinOPverwaltung") == 1) // AFR
                                                                                                            // zugelassen
                                                                                                            // ?
                    )) {
                ...
                buf2.append("x_nummer='" + vec2.get(z)
                                               .get(0)
                        + "', ");
                buf2.append("r_datum='" + vec2.get(z)
                                              .get(7)
                        + "', ");

                // Patienten-Name, Vorname holen und eintragen
                cmd = "select n_name, v_name from pat5 where id='" + vec2.get(z)
                                                                         .get(2)
                        + "'";
                vec.clear();
                vec = SqlInfo.holeFelder(cmd);
                if (vec.size() <= 0)
                    strHelp = "Patient, unbekannt";
                else
                    strHelp = vec.get(0)
                                 .get(0)
                            + ", " + vec.get(0)
                ...
                strHelp = vec2.get(z)
                              .get(1);
                buf2.append("r_klasse='" + strHelp.substring(0, 2) + "', "); // Hole die ersten beiden Buchstaben aus
                                                                             // der Rezeptnummer als "Klasse"

                buf2.append("r_betrag='" + vec2.get(z)
                                               .get(3)
                        + "', ");
                buf2.append("r_offen='" + vec2.get(z)
                                              .get(4)
                        + "', ");
                buf2.append("r_zuzahl='0.00', ");
                buf2.append("pat_intern='" + vec2.get(z)
                                                 .get(2)
                        + "', ");
                buf2.append("ikktraeger='" + vec2.get(z)
                                                 .get(1)
                        + "' "); // Rezept-Nummer ER23
...
//				System.out.println("Datensatz " + vec2.get(z).get(0) + " existiert bereits");
        ...
        JOptionPane.showMessageDialog(null,
                "<html>Folgende Rechnungs-Datensätze wurden kopiert:<br><font color=#0000FF>" + strKopiert
                        + "</font><br><br>"
    ...
    private void doSpeichern() {
        ...
        inif.setIntegerProperty("ZusaetzlicheRechnungen", "RGRinOPverwaltung",
                SystemConfig.hmZusatzInOffenPostenIni.get("RGRinOPverwaltung"), " RFR in OP-Verwaltung");
        inif.setIntegerProperty("ZusaetzlicheRechnungen", "AFRinOPverwaltung",
                SystemConfig.hmZusatzInOffenPostenIni.get("AFRinOPverwaltung"), " AFR in OP-Verwaltung");

        // Die sysetmspezifischen Daten müssen eigentlich in die Tabelle 'ini'
         ...
         * (!nummernkreisok ? "insert into nummern set " : "update nummern set ") ; int
         * edited = 0; for(int i = 0; i < 15; i++){ if(!
         * tfs[i].getText().trim().equals(originale.get(i))){ edited++; cmd =
         * cmd+(edited > 1 ? "," :
         * "")+tfs[i].getName()+"='"+tfs[i].getText().trim()+"'"; meldung =
         * meldung+tfs[i].getName()+" = "+tfs[i].getText().trim()+"\n"; } } if(edited >
         * 0){ cmd = cmd+ " LIMIT 1"; //cmd = cmd+(!nummernkreisok ? "" :
         * " where mandant = '"+Reha.aktIK+"' LIMIT 1"); meldung =
         * meldung+"\n\n"+"Diese Nummern abspeichern?"+"\n"; int frage =
         * JOptionPane.showConfirmDialog(null, meldung,
         * "Die geänderten Nummern abspeichern", JOptionPane.YES_NO_OPTION); if(frage ==
         * JOptionPane.YES_OPTION){ SqlInfo.sqlAusfuehren(cmd); }else{ for(int i = 0; i
         * < 15; i++){ tfs[i].setText(originale.get(i)); } } }else{
         * JOptionPane.showMessageDialog(
         * null,"Nummernkreis wurde nicht verändert (gute Entscheidung!!)");
         * SystemInit.abbrechen(); }
         */
    }
}
