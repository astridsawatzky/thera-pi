...
public class SysUtilRezepte extends JXPanel implements KeyListener, ActionListener {
    ...
    private void doSpeichern() {
        try {
            ...
            if (formok) {
                inif.setStringProperty("Formulare", "RezeptFormulareAnzahl", Integer.valueOf(rows)
                                                                                    .toString(),
                        null);
                for (int i = 0; i < rows; i++) {
                    inif.setStringProperty("Formulare", "RFormularText" + (i + 1), (String) vorlagen.getValueAt(i, 0),
                            null);
                    inif.setStringProperty("Formulare", "RFormularName" + (i + 1), (String) vorlagen.getValueAt(i, 1),
                            null);
                }
            }
