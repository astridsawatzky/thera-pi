...
public class SysUtilRoogleGruppen extends JXPanel implements ActionListener, KeyListener {
    ...
    private void kollegenEntfernen() {
        int i, lang;
        lang = glm.getDataVector()
                  .size();
        Vector vect = new Vector(vkollegen);
        // Vector vect = new Vector(klm.getDataVector());
        for (i = 0; i < lang; i++) {
            int x = vect.indexOf(glm.getDataVector()
                                    .get(i));

            if (x >= 0) {
                //// System.out.println("index = "+x+" entferne Kollege
                //// "+((Vector)vmitglieder.get(i)).get(0));
                vkollegen.removeElement(glm.getDataVector()
                                           .get(i));
                // klm.removeRow(x);
            }
        }
        ...
        try {
            ...
            if (RGmembers.getRowCount() == 0) {
                JOptionPane.showMessageDialog(null,
                        "Eine RoogleGruppe ohne Mitglieder kann nicht abgespeichert werden!");
                return;
            }
            if (RGname.getText()
                      .equals("")) {
            ...
            inianzahl = (!lneu ? Integer.valueOf(jcomboWahl.getItemCount())
                                        .toString()
                    : Integer.valueOf(jcomboWahl.getItemCount() + 1)
                             .toString());
            //// System.out.println(inianzahl);
            iniakt = (lneu ? Integer.valueOf(Integer.valueOf(inianzahl))
                                    .toString()
                    : Integer.valueOf(jcomboWahl.getSelectedIndex() + 1)
                             .toString());
            ...
            if (lneu) {
                SystemConfig.UpdateIni("terminkalender.ini", "RoogleEinstellungen", "RoogleAnzahlGruppen",
                        Integer.valueOf(inianzahl)
                               .toString());
            }
            SystemConfig.UpdateIni("terminkalender.ini", "RoogleEinstellungen", "RoogleNameGruppen" + iniakt, setXname);
            SystemConfig.UpdateIni("terminkalender.ini", "RoogleEinstellungen", "RoogleFelderGruppen" + iniakt,
                    inimitglied);
    ...
    private void comboFuellen(int akt) {
        ...
        for (i = 0; i < lang; i++) {
            gruppenname[i] = (String) ((ArrayList) SystemConfig.aRoogleGruppen.get(i)
                                                                              .get(0)).get(0);
            mitglieder[i] = SystemConfig.aRoogleGruppen.get(i)
                                                       .get(1)
                                                       .get(0);
    ...
    private void macheKollegen() {
        ...
        for (von = 1; von < bis; von++) {
            ...
            vec.add(String.valueOf(KollegenLaden.getMatchcode(von)));
    ...
    private void loeschenHandeln() {
        ...
        if (anzahlSets == 1) {
            JOptionPane.showMessageDialog(null,
                    "Diese Roogle-Gruppe ist die einzige Gruppe!\n" + "Die letzte Gruppe darf nicht gelöscht werden!");
            return;
        }

        int anfrage = JOptionPane.showConfirmDialog(null, "Wollen Sie dieses Gruppe wirlich löschen",
        ...
        if (anfrage == JOptionPane.YES_OPTION) {
            ...
            String set = "";
            SystemConfig.UpdateIni("terminkalender.ini", "RoogleEinstellungen", "RoogleAnzahlGruppen",
                    Integer.valueOf(anzahlSets - 1)
                           .toString());
            lspeichern = true;
            for (i = 0; i < anzahlSets; i++) {
                if (i != aktSet) {
                    // gruppenname[i] =
                    // (String)((ArrayList)SystemConfig.aRoogleGruppen.get(i).get(0)).get(0);
                    // mitglieder[i] =
                    // ((ArrayList<String[]>)SystemConfig.aRoogleGruppen.get(i).get(1)).get(0);
                    set = "";
                    int lang = mitglieder[i].length;
                    for (j = 0; j < lang; j++) {
                        set = set + (j > 0 ? "," + mitglieder[i][j] : mitglieder[i][j]);
                    }
                    SystemConfig.UpdateIni("terminkalender.ini", "RoogleEinstellungen",
                            "RoogleNameGruppen" + (++iniSet), gruppenname[i]);
                    SystemConfig.UpdateIni("terminkalender.ini", "RoogleEinstellungen",
                            "RoogleFelderGruppen" + (++iniSet), set);
                }
            }
            if (aktSet > 0) {
                aktSet = aktSet - 1;
