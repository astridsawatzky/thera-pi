...
public class SysUtilNachrichtenGruppen extends JXPanel implements ActionListener, KeyListener {
    ...
    private void speichernHandeln() {
        ...
        if (RGmembers.getRowCount() == 0) {
            JOptionPane.showMessageDialog(null,
                    "Eine Nachrichten-Gruppe ohne Mitglieder kann nicht abgespeichert werden!");
            return;
        }
        if (RGname.getText()
                  .equals("")) {
        ...
        for (int x = 0; x < RGmembers.getRowCount(); x++) {
            members = members
                    + (x > 0 ? ";" + (String) RGmembers.getValueAt(x, 0) : (String) RGmembers.getValueAt(x, 0));
        }

        String cmd = (lneu ? "insert into pimailgroup " : "update pimailgroup ") + "set groupname='" + setXname + "', "
                + "groupmembers='" + members + "'"
                + (lneu ? "" : " where id ='" + id[jcomboWahl.getSelectedIndex()] + "' LIMIT 1");
    ...
    private void comboFuellen(int akt) {
        ...
        id = new String[lang];
        for (i = 0; i < lang; i++) {
            gruppenname[i] = vecgruppen.get(i)
                                       .get(0);
            mitglieder[i] = vecgruppen.get(i)
                                      .get(1)
                                      .split(";");
            id[i] = vecgruppen.get(i)
                              .get(2);
        }
        // String[] test = new String[lang];
        /*
         * gruppenname = new String[lang]; mitglieder = new String[lang][]; id = new
         * String[lang];
         * 
         * for(i=0;i<lang;i++){ gruppenname[i] =
