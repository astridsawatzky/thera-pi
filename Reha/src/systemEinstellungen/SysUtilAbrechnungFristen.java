...
public class SysUtilAbrechnungFristen extends JXPanel implements KeyListener, ActionListener, FocusListener {
    ...
    private JPanel getVorlagenSeite() {
        ...
        builder.add(
                cmbtarife = new JRtaComboBox(
                        SystemPreislisten.hmPreisGruppen.get((defaultHM != null ? defaultHM : "Physio"))),
                cc.xyw(3, 5, 5));
        // builder.add(cmbtarife = new
        // JRtaComboBox(SystemPreislisten.hmPreisGruppen.get(xdisziplin[cmbdiszi.getSelectedIndex()])),
        // cc.xyw(3, 5,5));
        ...
        rads[2].setOpaque(false);
        rads[3].setOpaque(false);
        ...
        bgzuzahl.add(rads[4]);
        bgzuzahl.add(rads[5]);
        rads[4].setOpaque(false);
        rads[5].setOpaque(false);
    ...
    private void werteEinlesen() {
        lastdiszi = cmbdiszi.getSelectedItem()
                            .toString();
        lasttarif = cmbtarife.getSelectedIndex();

        tage1.setText(Integer.toString((Integer) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                       .get(0)).get(lasttarif)));
        if ((Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                            .get(1)).get(lasttarif)) {
            rads[0].setSelected(true);
        } else {
            rads[1].setSelected(true);
        }
        tage2.setText(Integer.toString((Integer) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                       .get(2)).get(lasttarif)));
        if ((Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                            .get(3)).get(lasttarif)) {
        ...
        if (hmZuzahlModus.get(lastdiszi)
                         .get(lasttarif) == 1) {
            rads[4].setSelected(true);
        } else {
            rads[5].setSelected(true);
        }
        cbox[0].setSelected((Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                                            .get(4)).get(lasttarif));
        cbox[1].setSelected((Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                                            .get(5)).get(lasttarif));
    }
    ...
    private boolean werteChanged() {
        ...
        try {
            if (Integer.parseInt(tage1.getText()) != (Integer) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                                     .get(0)).get(lasttarif)) {
                changed = true;
            }
            if (Integer.parseInt(tage2.getText()) != (Integer) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                                     .get(2)).get(lasttarif)) {
                changed = true;
            }
            if (rads[0].isSelected() != (Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                        .get(1)).get(lasttarif)) {
                changed = true;
            }
            if (rads[2].isSelected() != (Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                        .get(3)).get(lasttarif)) {
                changed = true;
            }
            if (rads[4].isSelected() != (hmZuzahlModus.get(lastdiszi)
                                                      .get(lasttarif) == 1 ? true : false)) {
                changed = true;
            }
            if (cbox[0].isSelected() != (Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                        .get(4)).get(lasttarif)) {
                changed = true;
            }
            if (cbox[1].isSelected() != (Boolean) ((Vector<?>) hmFristen.get(lastdiszi)
                                                                        .get(5)).get(lasttarif)) {
                changed = true;
            }
    ...
    private void doSpeichern() {
        try {
            ((Vector) hmFristen.get(lastdiszi)
                               .get(0)).set(lasttarif, Integer.parseInt(tage1.getText()));
            ((Vector) hmFristen.get(lastdiszi)
                               .get(2)).set(lasttarif, Integer.parseInt(tage2.getText()));
            ((Vector) hmFristen.get(lastdiszi)
                               .get(1)).set(lasttarif, rads[0].isSelected());
            ((Vector) hmFristen.get(lastdiszi)
                               .get(3)).set(lasttarif, rads[2].isSelected());
            ((Vector) hmFristen.get(lastdiszi)
                               .get(4)).set(lasttarif, cbox[0].isSelected());
            ((Vector) hmFristen.get(lastdiszi)
                               .get(5)).set(lasttarif, cbox[1].isSelected());
            ((Vector) hmZuzahlModus.get(lastdiszi)).set(lasttarif, (rads[4].isSelected() ? 1 : 0));
            SystemConfig.UpdateIni("fristen.ini", "Fristen_" + lastdiszi,
                    "FristBeginn" + Integer.toString(lasttarif + 1), tage1.getText());
            SystemConfig.UpdateIni("fristen.ini", "Fristen_" + lastdiszi,
                    "FristUnterbrechung" + Integer.toString(lasttarif + 1), tage2.getText());
            SystemConfig.UpdateIni("fristen.ini", "Fristen_" + lastdiszi,
                    "BeginnKalendertage" + Integer.toString(lasttarif + 1), (rads[0].isSelected() ? "1" : "0"));
            SystemConfig.UpdateIni("fristen.ini", "Fristen_" + lastdiszi,
                    "UnterbrechungKalendertage" + Integer.toString(lasttarif + 1), (rads[2].isSelected() ? "1" : "0"));
            SystemConfig.UpdateIni("fristen.ini", "Fristen_" + lastdiszi,
                    "BeginnMitSamstag" + Integer.toString(lasttarif + 1), (cbox[0].isSelected() ? "1" : "0"));
            SystemConfig.UpdateIni("fristen.ini", "Fristen_" + lastdiszi,
                    "UnterbrechungMitSamstag" + Integer.toString(lasttarif + 1), (cbox[1].isSelected() ? "1" : "0"));
            SystemConfig.UpdateIni("preisgruppen.ini", "ZuzahlRegeln_" + lastdiszi,
                    "ZuzahlModus" + Integer.toString(lasttarif + 1), (rads[4].isSelected() ? "1" : "0"));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (werteChanged()) {
            int frage = JOptionPane.showConfirmDialog(null,
                    "Sie haben die Werte verändert!\n" + "Sollen die Änderungen jetzt abgespeichert werden",
        ...
        if (e.getActionCommand()
             .equals("neuedisziplin")) {
            if (lasttarif >= 0) {
                try {
                    cmbtarife.removeActionListener(getInstance());
                    cmbtarife.setDataVector(
                            SystemPreislisten.hmPreisGruppen.get(xdisziplin[cmbdiszi.getSelectedIndex()]));
                    cmbtarife.setSelectedIndex(lasttarif);
                    cmbtarife.addActionListener(getInstance());
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(null,
                            "Fehler beim einstellen der Preisgruppe\n->" + xdisziplin[cmbdiszi.getSelectedIndex()]);
                }
            }
        }
        werteEinlesen();
