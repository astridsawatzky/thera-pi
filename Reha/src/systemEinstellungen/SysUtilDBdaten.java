...
public class SysUtilDBdaten extends JXPanel implements KeyListener, ActionListener {
    ...
    private JPanel getVorlagenSeite() {
        try {
            ...
            for (int i = 0; i < SystemConfig.Mandanten.size(); i++) {
                ...
                /*
                 * if(SystemConfig.Mandanten.get(i)[1].equals(Reha.aktMandant)){ vecindex = i; }
                 */
            }
            mandant.setSelectedItem(Betriebsumfeld.getAktMandant());
            ...
            try {
                dbtyp.setSelectedItem(SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                              .get(0));
            } catch (Exception ex) {
            ...
            //// System.out.println("DBType von "+Reha.aktMandant+"=
            //// "+SystemConfig.hmDBMandant.get(Reha.aktMandant).get(0));
            // System.out.println(SystemConfig.hmDBMandant.get(Reha.aktMandant));
            treiber = new JRtaTextField("", true);
            treiber.setText((String) SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                             .get(1));
            server = new JRtaTextField("", true);
            server.setText((String) SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                            .get(2));
            dbname = new JRtaTextField("", true);
            dbname.setText((String) SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                            .get(4));
            dbuser = new JRtaTextField("", true);
            dbuser.setText((String) SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                            .get(5));
            dbpasswort = new JPasswordField();
            dbpasswort.setText((String) SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                                .get(6));
            port = new JRtaTextField("ZAHLEN", true);
            port.setText((String) SystemConfig.hmDBMandant.get(Betriebsumfeld.getAktMandant())
                                                          .get(3));
            /*
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand()
            ...
            JOptionPane.showMessageDialog(null,
                    "Die Aufnahme einer 'Nicht-MySQL-Datenbank' wird erst später verfügbar sein");
        }
        if (e.getActionCommand()
             .equals("abbruch")) {
    ...
    private void speichernDBMandant() {
        ...
        /*
         * for(int i = 0; i < SystemConfig.Mandanten.size();i++){
         * if(SystemConfig.Mandanten.get(i)[1].equals(smandant)){ siniverz =
         * SystemConfig.Mandanten.get(i)[0]; break; } }
         */
        ...
        ss6 = (dbuser.getText()
                     ...
                     .equals("") ? ""
                             : dbuser.getText()
                                     .trim());
        ss7 = (String.valueOf(dbpasswort.getPassword())
                     .trim()
                     .equals("") ? ""
                             : String.valueOf(dbpasswort.getPassword())
                                     .trim());
        if (ss2.equals("keineAngaben") || ss3.equals("keineAngaben") || ss4.equals("keineAngaben")
                || ss5.equals("keineAngaben")) {
        ...
        if (saktmandant.equals(Betriebsumfeld.getAktMandant())) {
            JOptionPane.showMessageDialog(null,
                    "Die Datenbankeinstellungen für den aktuellen Mandant,\n"
    ...
    private void neuerDBMandant() {
        ...
        String smandant = ((String) mandant.getSelectedItem()).trim();
        dbtyp.setSelectedItem(SystemConfig.hmDBMandant.get(smandant)
                                                      .get(0));
        treiber.setText((String) SystemConfig.hmDBMandant.get(smandant)
                                                         .get(1));
        server.setText((String) SystemConfig.hmDBMandant.get(smandant)
                                                        .get(2));
        dbname.setText((String) SystemConfig.hmDBMandant.get(smandant)
                                                        .get(4));
        dbuser.setText((String) SystemConfig.hmDBMandant.get(smandant)
                                                        .get(5));
        dbpasswort.setText((String) SystemConfig.hmDBMandant.get(smandant)
                                                            .get(6));
        port.setText((String) SystemConfig.hmDBMandant.get(smandant)
                                                      .get(3));
        validate();
    }
    ...
    private void testenDBMandant(String selectedMandantIK) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,
                    "Die Treiberklasse der Datenbank wurde nicht gefunden, sind die Angaben korrekt?");
            return;
        }
        try {
