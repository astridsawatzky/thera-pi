...
public class SysUtilDruckvorlage extends JXPanel implements KeyListener, ActionListener, FocusListener {
    ...
    private JPanel getVorlagenSeite() {
        ...
        if (SystemConfig.oTerminListe.NameTerminDrucker.trim()
                                                       .equals("")) {
        ...
        tabanz.setText(Integer.valueOf(SystemConfig.oTerminListe.AnzahlTerminTabellen)
                              .toString());
        zeilanz = new JRtaTextField("ZAHLEN", true);
        zeilanz.setText(Integer.valueOf(SystemConfig.oTerminListe.AnzahlTermineProTabelle)
                               .toString());
        spaltanz = new JRtaTextField("ZAHLEN", true);
        spaltanz.setText(Integer.valueOf(SystemConfig.oTerminListe.AnzahlSpaltenProTabellen)
                                .toString());
        ...
        int anz = Integer.valueOf(spaltanz.getText()
                                          .trim());
        for (int i = 0; i < 4; i++) {
            if (i < anz) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand()
            ...
            thisClass.setCursor(Cursors.wartenCursor);
    ...
    private void datenSpeichern() {
        ...
        int test1 = Integer.valueOf(tabanz.getText()
                                          .trim());
        if (test1 == 0) {
            JOptionPane.showMessageDialog(null,
                    "Sie wollen eine Definition abspeichern mit Anzahl Tabelle = 0,\n\n"
                            + "Mit diesem IQ sind Sie ein Fall für THEORG! Diese Definition wird jedenfalls\n"
                            + "nicht abgespeichert!");
            return;
        }
        int test2 = Integer.valueOf(spaltanz.getText()
                                            .trim());
        if (test2 == 0) {
            JOptionPane.showMessageDialog(null,
                    "Sie wollen eine Definition abspeichern mit Anzahl Spalten = 0,\n\n"
                            + "Mit diesem IQ sind Sie ein Fall für THEORG! Diese Definition wird jedenfalls\n"
                            + "nicht abgespeichert!");
            return;
        }
        int test3 = Integer.valueOf(zeilanz.getText()
                                           .trim());
        if (test3 == 0) {
            JOptionPane.showMessageDialog(null,
                    "Sie wollen eine Definition abspeichern mit Terminzeile pro Tabelle  = 0,\n\n"
                            + "Mit diesem IQ sind Sie ein Fall für THEORG! Diese Definition wird jedenfalls\n"
                            + "nicht abgespeichert!");
            return;
        }
        try {
            ...
            ini.setStringProperty("TerminListe1", "AnzahlSpaltenProTabellen", Integer.valueOf(test2)
                                                                                     .toString(),
                    null);
            SystemConfig.oTerminListe.AnzahlSpaltenProTabellen = test2;

            ini.setStringProperty("TerminListe1", "AnzahlTermineProTabelle", Integer.valueOf(test3)
                                                                                    .toString(),
                    null);
            SystemConfig.oTerminListe.AnzahlTermineProTabelle = test3;

            ini.setStringProperty("TerminListe1", "NameTemplate", vorlagenname.getText()
                                                                              .trim(),
                    null);
            SystemConfig.oTerminListe.NameTemplate = vorlagenname.getText()
                                                                 .trim();

            ini.setStringProperty("TerminListe1", "NameTerminDrucker", ((String) druckername.getSelectedItem()).trim(),
                    null);
            ...
            ini.setStringProperty("TerminListe1", "MitSpaltenUeberschrift", (spaltenkopf.isSelected() ? "1" : "0"),
                    null);
            ...
            for (int i = 0; i < 4; i++) {
                if (spa[i].isEnabled()) {
                    ini.setStringProperty("TerminListe1", "InhaltSpalte" + (i + 1), spalten[spa[i].getSelectedIndex()],
                            null);
     ...
     * "AnzahlSpaltenProTabellen")); for(int i = 0;i<AnzahlSpaltenProTabellen;i++){
     * NamenSpalten.add(RWJedeIni.leseIniDatei(iniName, "TerminListe1",
     * "InhaltSpalte"+(i+1)) ); } AnzahlTermineProTabelle =
     * Integer.valueOf(RWJedeIni.leseIniDatei(iniName, "TerminListe1",
     * "AnzahlTermineProTabelle")); NameTemplate = RWJedeIni.leseIniDatei(iniName,
     * "TerminListe1", "NameTemplate"); NameTerminDrucker =
     * RWJedeIni.leseIniDatei(iniName, "TerminListe1", "NameTerminDrucker");
     * PatNameDrucken = Integer.valueOf(RWJedeIni.leseIniDatei(iniName,
     * "TerminListe1", "PatNameDrucken")); PatNamenPlatzhalter =
     * RWJedeIni.leseIniDatei(iniName, "TerminListe1", "PatNamePlatzhalter");
     * MitUeberschrift = Integer.valueOf(RWJedeIni.leseIniDatei(iniName,
     * "TerminListe1", "MitSpaltenUeberschrift")); }
     */
    private void dateiDialog() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            if (inputVerzFile.getName()
                ...
                vorlagenname.setText(inputVerzFile.getName()
                                                  .trim());
            }
        } else {
            vorlagenname.setText(SystemConfig.oTerminListe.NameTemplate);
        ...
        if (((JComponent) arg0.getSource()).getName() != null) {
            if (((JComponent) arg0.getSource()).getName()
                ...
                if (spaltanz.getText()
                    ...
                    anzahl = Integer.valueOf(spaltanz.getText()
                                                     .trim());
                }
                if (anzahl == 0) {
    ...
    private void testDruck() {
        ...
        /*
         * JRtaTextField tabanz = null; JRtaTextField zeilanz = null; JRtaTextField
         * spaltanz = null;
         */
        int AnzahlTabellen = Integer.valueOf(tabanz.getText()
                                                   .trim());
        if (AnzahlTabellen == 0) {
        ...
        int maxTermineProTabelle = Integer.valueOf(zeilanz.getText()
                                                          .trim());
        if (maxTermineProTabelle == 0) {
            JOptionPane.showMessageDialog(null,
                    "Mit der Angabe Termine pro Tabelle == 0 -> kein Terminplan und deshalb auch kein Test!");
            thisClass.setCursor(Cursors.normalCursor);
            return;
        }
        int maxTermineProSeite = AnzahlTabellen * maxTermineProTabelle;
        int spaltenProtabelle = Integer.valueOf(spaltanz.getText()
                                                        .trim());
        if (spaltenProtabelle == 0) {
            thisClass.setCursor(Cursors.normalCursor);
            JOptionPane.showMessageDialog(null,
                    "Ohne Spalten keine Tabelle, ohne Tabelle kein Terminplan -> und deshalb auch kein Test!");
            return;
        }
        String[] spalten = { "Wochentag", "Datum", "Uhrzeit", "Behandler" };
        ...
        if (tbl.length != AnzahlTabellen) {
            JOptionPane.showMessageDialog(null,
                    "Anzahl Tabellen stimmt nicht mit Ihren Angaben überein.\nDer Test wird abgebrochen!");
            textDocument.close();
            thisClass.setCursor(Cursors.normalCursor);
            return;
        }
        ...
        if (ipatdrucken > 0) {
             ...
             * //Selektion durch eigenen Text ersetzen textRanges[0].setText(patname);
             * }else{ thisClass.setCursor(Reha.instance.normalCursor);
             * JOptionPane.showMessageDialog (null, "Suche nach ^Name^ war erfolglos"); }
             */
            /*
             * ITextService textService = textDocument.getTextService(); IBookmarkService
             * bookmarkService = textService.getBookmarkService(); IBookmark bookmark =
        ...
        try {
            // textTable =
            // textDocument.getTextTableService().getTextTable(SystemConfig.oTerminListe.NameTabelle[0]);
        ...
        while (true) {
            ...
            /********************/
            /*
             * public String tag; public String beginn; public String termtext; public
             * String sorter;
             */
            // **************/Hier die Zellen*************//
