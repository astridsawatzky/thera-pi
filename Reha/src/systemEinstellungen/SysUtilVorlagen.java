...
//			vec.add(inif.getStringProperty("Formulare","AFormularName"+i));
    ...
    public boolean saveToIni() {
        ...
        if (formok) {
            inif.setStringProperty(sectionLabel, nbOfEntriesLabel, Integer.valueOf(rows)
                                                                          .toString(),
                    null);
            for (int i = 0; i < rows; i++) {
                inif.setStringProperty(sectionLabel, entryTextLabel + (i + 1), (String) vorlagen.getValueAt(i, 0),
                        null);
                inif.setStringProperty(sectionLabel, entryNameLabel + (i + 1), (String) vorlagen.getValueAt(i, 1),
                        null);
            }
        }
        return formok;
