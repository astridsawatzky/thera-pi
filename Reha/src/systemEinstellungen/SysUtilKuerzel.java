...
public class SysUtilKuerzel extends JXPanel implements ActionListener {
    ...
    private JPanel getVorlagenSeite() {
        ...
        if (SystemConfig.mitRs) {
            disziplin = new JRtaComboBox(
                    new String[] { "Physio", "Massage", "Ergo", "Logo", "Reha", "Podo", "Rsport", "Ftrain" });
        } else {
        ...
        modkuerzel.setColumnIdentifiers(
                new String[] { "Kürzel", "Langtext", "Disziplin", "vorrangig", "isoliert", "id" });
        tblkuerzel = new JXTable(modkuerzel);
        MitteRenderer mr = new MitteRenderer();
        tblkuerzel.getColumn(0)
                  .setMaxWidth(80);
        tblkuerzel.getColumn(2)
                  .setMaxWidth(80);
        tblkuerzel.getColumn(2)
                  .setCellRenderer(mr);
        tblkuerzel.getColumn(3)
                  .setMaxWidth(60);
        tblkuerzel.getColumn(4)
                  .setMaxWidth(60);
        tblkuerzel.getColumn(5)
                  .setMaxWidth(60);
        tblkuerzel.getColumn(5)
                  .setCellRenderer(mr);
    ...
    private void tabelleFuellen(String diszi, int row) {
        ...
        for (int i = 0; i < lang; i++) {
            ...
            dummy.add(vec.get(i)
                         .get(this.I_LEISTUNG));
            dummy.add(vec.get(i)
                         .get(this.I_DISZIPLIN));
            dummy.add(vec.get(i)
                         .get(this.I_VORRANGIG)
                         .equals("T") ? true : false);
            dummy.add(vec.get(i)
                         .get(this.I_EXTRAOK)
                         .equals("T") ? true : false);
    ...
    public Object[] getKuerzelDaten() {
        ...
        aktuelleID = modkuerzel.getValueAt(tblkuerzel.convertRowIndexToModel(row), this.I_ID)
                               .toString();
        aktuelleRow = row;
        ret[0] = modkuerzel.getValueAt(tblkuerzel.convertRowIndexToModel(row), 0)
                           .toString();
        ret[1] = modkuerzel.getValueAt(tblkuerzel.convertRowIndexToModel(row), 1)
                           .toString();
    ...
    public void updateKuerzel(String kurz, String lang, Boolean vorrang, Boolean separat) {
        String cmd = "update kuerzel set kuerzel='" + kurz + "', leistung='" + lang + "', " + "vorrangig='"
                + (vorrang ? "T" : "F") + "', extraok='" + (separat ? "T" : "F") + "'" + " where id='" + aktuelleID
                + "' LIMIT 1";
    ...
    public void insertKuerzel(String kurz, String lang, Boolean vorrang, Boolean separat) {
        boolean gibtsschon = SqlInfo.gibtsSchon("select kuerzel from kuerzel where kuerzel='" + kurz
                + "' and disziplin='" + diszi[disziplin.getSelectedIndex()] + "'");
        if (gibtsschon) {
        ...
        String cmd = "insert into kuerzel set kuerzel='" + kurz + "', leistung='" + lang + "', disziplin='"
                + diszi[disziplin.getSelectedIndex()] + "', vorrangig='" + (vorrang ? "T" : "F") + "', extraok='"
                + (separat ? "T" : "F") + "'";
        SqlInfo.sqlAusfuehren(cmd);
        tabelleFuellen(diszi[disziplin.getSelectedIndex()], -1);
        for (int i = 0; i < tblkuerzel.getRowCount(); i++) {
            if (tblkuerzel.getValueAt(i, 0)
                          .toString()
                          .equals(kurz)) {
