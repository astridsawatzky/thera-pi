...
public class BenutzerRechte extends JXPanel {
    ...
    String[] hauptGruppen = { "Benutzer-Verwaltung", "Patiente", "Rezepte", "Historie", "Therapieberichte",
            "Dokumentation", "Gutachten", "Terminkalender", "Arbeitszeitmasken", "[Ru:gl]",
            "Abrechnung / Statistik / Umsatz", "System-Initialisierung", "Sonstige Programme und Funktionen" };

    String[] gruppe0 = { "Benutzer-Verwaltung öffnen", "Benutzer anlegen/ändern/löschen",
            "Benutzer hat alle Rechte = SuperUser" };
    ...
    String[] gruppe2 = { "anlegen", "ändern", "löschen", "Gebühren kassieren", "Ausfallrechnung erstellen",
            "Rezept abschließen", "Rezept aufschließen", "Privat-/BG-Rechnung erstellen",
            "Therapieberichte erstellen" };

    String[] gruppe3 = { "Gesamtumsatz einsehen", "einzelne Behandlungstage drucken",
            "nachträglich Therapie-Berichte erstellen/ändern" };
    ...
    String[] gruppe7 = { "nur in leere Termine eintragen", "Termine vollständig eintragen",
            "bestehende Termine löschen", "Behandlungen bestätigen", "Termine gruppieren erlauben",
            "Drag & Drop erlauben", "Behandlungen im nachhinein bestätigen",
            "unbelegt 2 für zukünftige Erweiterungen" };

    String[] gruppe8 = { "Masken erstellen / ändern", "Masken in Kalender übertragen" };

    String[] gruppe9 = { "[Ru:gl] öffnen", "Termine überschreiben", "unbelegt 1 für zukünftige Erweiterungen",
            "unbelegt für 2 zukünftige Erweiterungen" };

    String[] gruppe10 = { "Kassenabrechnung", "Rehaabrechnung", "Barkassen Abrechnung", "Neuanmeldungen ermitteln",
            "Umsätze von bis ermitteln", "Mitarbeiterbeteiligung ermitteln", "Urlaub-/Überstunden ermitteln",
            "Offene Posten / Mahnwesen", "Kassenbuch anlegen / ändern", "Rez. Gebühr bezahlt/unbezahlt ändern",
            "unbelegt 4 für zukünftige Erweiterungen", "unbelegt 5 für zukünftige Erweiterungen",
            "unbelegt 6 für zukünftige Erweiterungen", "unbelegt 7 für zukünftige Erweiterungen",
            "unbelegt 8 für zukünftige Erweiterungen", "unbelegt 9 für zukünftige Erweiterungen",
            "unbelegt 10 für zukünftige Erweiterungen" };

    String[] gruppe11 = { "Firmenangaben Mandanten", "Datenbankparameter", "Kalender Grundeinstellungen",
            "Kalender-Benuter verwalten", "Behandlersets einstellen", "Druckvorlage für Terminliste",
            "Kalender-Farbdefinition", "Gruppentermine verwalten", "Neues Kalenderjahr anlegen",
            "[Ru:gl]-Grundeinstellungen", "[Ru:gl]-Gruppen definieren", "Einstellungen Patientenfenster",
            "Einstellungen Rezepte", "Einstellungen Krankenfenster", "Einstellungen Arztfenster",
            "Emailparameter ändern", "Geräteanschlüsse (Schnittstellen)", "angeschlossene Geräte",
            "Behandlunskürzel verwalten", "Tarifgruppen bearbeiten", "Preise bearbeiten/importieren",
            "Nummernkreise verwalten", "Nebraska benützen", "Abrechnungsformulare und Druckparameter",
            "Kostenträgerdatei einlesen", "Fremdprogramme verwalten", "unbelegt 1 für zukünftige Erweiterungen",
            "unbelegt 2 für zukünftige Erweiterungen", "unbelegt 3 für zukünftige Erweiterungen",
            "unbelegt 4 für zukünftige Erweiterungen", "unbelegt 5 für zukünftige Erweiterungen" };

    String[] gruppe12 = { "Verkaufsmodul benutzen", "Rehaformulare verwenden",
            "Textbausteine für Gutachten anlegen/ändern", "Rezepte in Historie schieben und umgekehrt",
            "im Rezept gespeicherte Behandlungsarten löschen", "Geburtstagsbriefe erstellen", "Sql-Modul verwenden",
            "Reha-Fallsteuerung nach §301", "Darf Thera-Pi-Nachrichten löschen",
            "Artikel in Verkaufsmodul-Modul anlegen", "unbelegt 8 für zukünftige Erweiterungen",
            "unbelegt 9 für zukünftige Erweiterungen", "unbelegt 10 für zukünftige Erweiterungen" };
    /*************************************/
    ...
    private JScrollPane getTreeTableTeil() {
         ...
         * comborechte.component.addItem(lab); lab = new JLabel();
        ...
        TableCellRenderer renderer = new DefaultTableRenderer(new MappedValue(StringValues.EMPTY, IconValues.ICON),
                JLabel.CENTER);
        jXTreeTable.getColumn(1)
                   .setCellRenderer(renderer);
        jXTreeTable.getColumn(1)
                   .setMaxWidth(100);
        jXTreeTable.getColumn(1)
                   .setCellEditor(comborechte);
    ...
    private void regleButtons(String enable) {
        for (int i = 0; i < buts.length; i++) {
            buts[i].setEnabled((enable.substring(i, i + 1)
                                      .equals("1") ? true : false));
        }
        if (!rechteTools.Rechte.hatRecht(rechteTools.Rechte.BenutzerSuper_user, false)) {
    ...
    private void doBenutzerWahl() {
        if (jcmb.getSelectedIndex() == 0) {
            ...
            tfs[0].setText(jcmb.getSelectedItem()
                               .toString());
            pws[0].setText(jcmb.getValue()
                               .toString());
            pws[1].setText(jcmb.getValue()
                               .toString());
            ...
            if (jchb.isSelected()) {
                this.internal.setTitle(elternTitel + " [PW:" + jcmb.getValue()
                                                                   .toString()
                        + "]");
            } else {
                this.internal.setTitle(elternTitel);
    ...
    private void doImport() {
        ...
        RechteImport rimport = new RechteImport(buts[6].getLocationOnScreen(), importRechte, "Rechte-Gruppe auswählen",
                vec);
        rimport.pack();
        rimport.setModal(true);
        rimport.setVisible(true);
        if (!importRechte.getText()
                         .equals("")) {
    ...
    private void doExport() {
        ...
        Object ret = JOptionPane.showInputDialog(null, "Geben Sie bitte einen Namen für die Rechte-Gruppe ein",
                rechtegruppe);
        if (ret == null) {
            return;
        }
        ...
        String cmd = "insert into restricttemplates set abteilung='" + man.encrypt(ret.toString()) + "', sammlung='"
                + man.encrypt(rechte) + "'";
    ...
    private void doEdit() {
        if (jcmb.getSelectedIndex() == 0) {
            // new AaarghHinweis("Und welcher Benutzer soll geändert werden?","nicht zu
            // fassen...");
    ...
    private void doDelete() {
        ...
        } else if (!jcmb.getSelectedItem()
                        .toString()
                        .trim()
                        .equals(Reha.aktUser)) {
            String user = jcmb.getSelectedItem()
                              .toString();
            int anfrage = JOptionPane.showConfirmDialog(null,
                    "Sie sind im Begriff einen Thera-Pi-Benutzer zu löschen!!!!\n\nWollen Sie  den Benutzer --> " + user
                            + " <-- wirklich löschen?\n\n",
                    "Achtung wichtige Benutzeranfrage", JOptionPane.YES_NO_OPTION);
            if (anfrage == JOptionPane.YES_OPTION) {
                String id = (String) jcmb.getValueAt(4);
    ...
    private void doSave() {
        ...
        if (!String.valueOf(pws[0].getPassword())
                   .equals(String.valueOf(pws[1].getPassword()))) {
        ...
        if (!neu) {
            ...
            String cmd = "update rehalogin set user='" + man.encrypt(tfs[0].getText()) + "', password='"
                    + man.encrypt(String.valueOf(pws[0].getPassword())) + "', rights='" + encrypted + "' where id='"
                    + userid + "' LIMIT 1";
            SqlInfo.sqlAusfuehren(cmd);
            ...
            if (Reha.aktUser.equals(jcmb.getSelectedItem()
                                        .toString()
                                        .trim())) {
            ...
            String test = String.valueOf(pws[0].getPassword())
                                .trim();
            ...
            for (int i = 0; i < size; i++) {
                if (test.equals(Benutzer.pKollegen.get(i)
                    ...
                    return;
                                                               ...
                                                               .trim())) {
                    JOptionPane.showMessageDialog(null,
                            "Benutzername " + name + " ist bereits vergeben, speichern nicht möglich");
                    return;
                }
            }
            /***********/
            ...
            if (tfs[0].getText()
                      .equals("")
                    || String.valueOf(pws[0].getPassword())
                             .trim()
                             .equals("")) {
