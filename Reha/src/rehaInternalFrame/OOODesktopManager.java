...
public class OOODesktopManager extends DefaultDesktopManager {
    ...
    void activateNextFrame(Container c) {
        ...
        for (i = 0; i < c.getComponentCount(); i++) {
            ...
            if (c.getComponent(i) instanceof JInternalFrame && (!c.getComponent(i)
                                                                  .getName()
                                                                  .equals(frameaktuell))) {
        ...
        /*
         * for(int y = 0; y < c.getComponentCount();y++){ if((c.getComponent(y)
         * instanceof JInternalFrame)){ String status =
         * "Komponente = "+c.getComponent(y).getName().toString()+" / frameaktuell = "+
         * frameaktuell+" / isIcon = "+ ((JRehaInternal)c.getComponent(y)).isIcon;
         * System.err.println(status); } } System.out.println(nextFrame);
         */
        if (nextFrame != null) {
            try {
