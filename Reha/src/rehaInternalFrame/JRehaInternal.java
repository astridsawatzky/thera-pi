...
public class JRehaInternal extends JInternalFrame implements ActionListener, ComponentListener, KeyListener,
  ...
  //  public String titel;
    ...
    public JRehaInternal(String titel, ImageIcon img, int desktop) {
        ...
        this.getContentPane()
            .setSize(new Dimension(this.getContentPane()
                                       .getWidth(),
                    this.getContentPane()
                        .getHeight() - 20));
        this.getContentPane()
                    ...
                    if (!lays.equals(frame)) {
                        ...
                        try {
                            ...
                            if (Reha.instance.desktops[idesk].getComponent(layers) instanceof JDesktopIcon) {
                                ...
                                System.out.println(
                                        "icon = false " + ((JRehaInternal) Reha.instance.desktops[idesk].getComponent(
                                                layers)).getName());
                                iframe = ((JRehaInternal) Reha.instance.desktops[idesk].getComponent(layers));
                            }
                        }
    ...
    public void showPopUp(java.awt.event.MouseEvent evt) {
        //// System.out.println("Aufruf des PopUpMenues an der Stelle X="+evt.getX()+" /
        //// Y="+evt.getY());
    ...
    private JPopupMenu getTerminPopupMenu() {
        ...
        jmen.setText((this.desktop == 0
                ? "in den " + (SystemConfig.desktopHorizontal ? "unteren" : "rechten") + " Container verschieben"
                : "in den " + (SystemConfig.desktopHorizontal ? "oberen" : "linken") + " Container verschieben"));

        jmen.setIcon((this.desktop == 0
                ? new ImageIcon(Path.Instance.getProghome()
                        + (SystemConfig.desktopHorizontal ? "/icons/unten.gif" : "/icons/forward.gif"))
                : new ImageIcon(Path.Instance.getProghome()
                        + (SystemConfig.desktopHorizontal ? "/icons/oben.gif" : "/icons/backward.gif"))));
        ...
        JCheckBoxMenuItem cbMenuItem = new JCheckBoxMenuItem(
                "stets versuchen das Fenster auf maximale Größe einstellen");
        ...
        jmen.setIcon(new ImageIcon(Path.Instance.getProghome() + "/icons/buttongreen.png"));
        jmen.setEnabled(true);
        ...
        jmen.setIcon(new ImageIcon(Path.Instance.getProghome() + "/icons/buttonred.png"));
        jmen.setEnabled(true);
    ...
    private void sizeAnpassen() {
        ...
        this.setSize(Reha.instance.desktops[this.desktop].getWidth() - 2,
                Reha.instance.desktops[this.desktop].getHeight() - 2);
    }
