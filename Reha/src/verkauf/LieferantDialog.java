...
public class LieferantDialog extends RehaSmartDialog {
    ...
    public LieferantDialog(int id, Point position) {
        ...
        /*
         * this.add(getJXTitledPanel(), BorderLayout.NORTH); this.setLocation((int)
         * (position.getX() - (this.getWidth() / 2)), (int) (position.getY() + 20));
    ...
    private void speicher() {
        ...
        if (!this.name.getText()
            ...
            if (this.lieferant == null) {
                this.lieferant = new Lieferant(name.getText(), ansprechpartner.getText(), anschrift.getText(),
                        plz.getText(), ort.getText(), telefon.getText(), telefax.getText());
            } else {
