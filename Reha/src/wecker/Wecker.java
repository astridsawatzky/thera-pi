...
public class Wecker extends RehaSmartDialog {
    ...
    public static void testeWecker() {
        ...
        try {
            ...
            for (int i = 0; i < Reha.timerVec.size(); i++) {
                if (aktuelleMinuten() >= (Long) Reha.timerVec.get(i)
                                                             .get(3)) {
                    abgelaufen = true;
                    termin = i;
                    break;
            ...
            if (abgelaufen) {
                ...
                if (Reha.timerVec.size() <= 0) {
                    ...
                    String msg = Long.toString((Long) Reha.timerVec.get(0)
                                                                   .get(3)
                            - aktuelleMinuten());
                    Reha.instance.messageLabel.setForeground(Color.RED);
                    Reha.instance.messageLabel.setText(
                            "in " + msg + (msg.equals("1") ? " Minute " : " Minuten ") + "nächster Timer-Termin");
                }
    ...
    private void doNeuTermin() {
        int minuten = 0;
        try {
            minuten = Integer.parseInt(tfs[2].getText()
                                             .trim());
            if (minuten <= 0) {
    ...
    static long aktuelleMinuten() {
        ...
        String zeitstring = (Integer.toString(hour)
                                    .length() < 2 ? "0" + Integer.toString(hour) : Integer.toString(hour))
                + ":" + (Integer.toString(min)
                                .length() < 2 ? "0" + Integer.toString(min) : Integer.toString(min))
                + ":00";
...
final class TerminAbgelaufen extends RehaSmartDialog {
    ...
    private void testeOk() {
        ...
        if (pwkorrekt) {
            String cmd = "insert into wecker set n_name='" + vec.get(2) + "', " + "raum='" + vec.get(1) + "', "
                    + "fertig='" + vec.get(0) + "', " + "registervon='" + pwuser + "', " + "registriert='"
                    + ZeitFunk.MinutenZuZeit(Integer.parseInt(Long.toString(Wecker.aktuelleMinuten()))) + "', "
    ...
    private void setzeTon() {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                new AePlayWave(Path.Instance.getProghome() + "sounds/" + "carhorn.wav").start();
                return null;
            }
        }.execute();
    }
    ...
    private String getHtmlText() {
        ...
        buf1.append("<tr>");
        buf1.append("<th rowspan=\"4\"><a href=\"http://rezedit.de\"><img src='file:///" + Path.Instance.getProghome()
