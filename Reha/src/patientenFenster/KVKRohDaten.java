...
public class KVKRohDaten extends RehaSmartDialog implements ActionListener {
    ...
    private JXPanel getTextPanel() {
         ...
         * "- Daten der Versicherungskarte -<br><br>" + "</font>"+
         */
                "<table align=center>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Krankenkasse:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Krankenkasse") + "</font>"
                + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "IK der Krankenkasse:" + "</font>" + "</td>\n"
                + "<td>\n" + "<font color=red>" + "<font size=+1>" + "10" + SystemConfig.hmKVKDaten.get("Kassennummer")
                + "</font>" + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Status:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Status")
                                                                                 .substring(0, 1)
                + "</font>" + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Versichertennummer:" + "</font>" + "</td>\n"
                + "<td>\n" + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Versichertennummer")
                + "</font>" + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Vorname:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Vorname") + "</font>"
                + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Nachname:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Nachname") + "</font>"
                + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Geboren:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + gerGeboren + "</font>" + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Strasse:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Strasse") + "</font>"
                + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Postleitzahl:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Plz") + "</font>"
                + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Ort:" + "</font>" + "</td>\n" + "<td>\n"
                + "<font color=red>" + "<font size=+1>" + SystemConfig.hmKVKDaten.get("Ort") + "</font>"
                + "</td></tr>\n" +
                //
                "<tr>\n" + "<td align=right>\n" + "<font size=+1>" + "Karte gültig bis:" + "</font>" + "</td>\n"
                + "<td>\n" + "<font color=blue><b>" + "<font size=+1>";
        try {
            initialText = initialText + monate[IntegerTools.trailNullAndRetInt(gueltig.substring(0, 2)) - 1] + " - 20"
                    + gueltig.substring(2);
        } catch (Exception ex) {
            initialText = initialText + "keine Angaben";
        }
        initialText = initialText + "</b>" + "</font>" + "</td></tr>\n" +
    ...
    private void werteUebernehmen() {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                if (!thisPat.inNeu) {
                    String test1 = thisPat.jtf[11].getText()
                                                  .trim();
                ...
                String[] list = {};
                // Für den Echtbetrieb!
                String kassik = "10" + SystemConfig.hmKVKDaten.get("Kassennummer")
                                                              .trim();
                Vector<String> vec = SqlInfo.holeSatz("kass_adr", "kassen_nam1,ik_kasse,id",
                        "ik_kasse='" + kassik + "'", Arrays.asList(list));

                if (vec.size() == 0) {
                    Vector<String> vec2 = SqlInfo.holeSatz("ktraeger", " * ", "ikkasse='" + kassik + "'",
                            Arrays.asList(list));
                    if (vec2.size() > 0) {
                        ...
                        JOptionPane.showMessageDialog(null,
                                "Die Krankenkasse ist weder im Kassenstamm noch in der Kostenträgerdatei vorhanden!\n"
                ...
                if ((SystemConfig.hmKVKDaten.get("Anrede") != null) && (!SystemConfig.hmKVKDaten.get("Anrede")
                                                                                                .equals(""))
                        && OcKVK.lastCardIsEGK) {
                ...
                if ((SystemConfig.hmKVKDaten.get("Titel") != null) && (!SystemConfig.hmKVKDaten.get("Titel")
                                                                                               .equals(""))) {
                ...
                try {
                    if (SystemConfig.hmKVKDaten.get("Status")
                                               .startsWith("0")
                            || SystemConfig.hmKVKDaten.get("Status")
                                                      .trim()
                                                      .length() == 0) {
                        thisPat.jtf[15].setText("1");
                    } else {
                        thisPat.jtf[15].setText(SystemConfig.hmKVKDaten.get("Status")
                                                                       .substring(0, 1));
                    }
                } catch (Exception ex) {
                    thisPat.jtf[15].setText("1");
                ...
                thisPat.jtf[2].setText(SystemConfig.hmKVKDaten.get("Nachname")
                                                              .toUpperCase());
                thisPat.jtf[3].setText(SystemConfig.hmKVKDaten.get("Vorname")
                                                              .toUpperCase());
                thisPat.jtf[4].setText(SystemConfig.hmKVKDaten.get("Strasse")
                                                              .toUpperCase());
                thisPat.jtf[5].setText(SystemConfig.hmKVKDaten.get("Plz")
                                                              .toUpperCase());
                thisPat.jtf[6].setText(SystemConfig.hmKVKDaten.get("Ort")
                                                              .toUpperCase());
    ...
    private int kasseAnlegen(Vector<String> vec) {
        ...
        if (ikpapier.trim()
                    .equals("")) {
            ikpapier = SqlInfo.holeEinzelFeld(
                    "select ikpapier from ktraeger where ikkasse='" + ikktraeger + "' LIMIT 1");
        }
        if (email.trim()
                 .equals("")
                && (!ikdaten.trim()
        ...
        String cmd = "<html><b>Die Kasse ist im aktuellen Kassenstamm nicht enthalten!<br><br>"
                + "Die Kasse wurde jedoch in der Kostenträgerdatei gefunden!" + "<br><br><font color=blue>" + name1
                + "<br>" + name2 + "<br>" + strasse + "<br>" + plz + " " + ort + "<br><br></font></b>" + "IK-Kasse: "
                + ikkasse + "<br>" + "IK-Kostenträger: " + ikktraeger + "<br>" + "IK-Nutzer für Entschl.: " + iknutzer
                + "<br>" + "IK-Datenannahmestelle: " + ikdaten + "<br>" + "Email Datenannahmest.: " + email + "<br>"
        ...
        if (anfrage == JOptionPane.YES_OPTION) {
            try {
                String[] list = new String[SystemPreislisten.hmPreisGruppen.get("Common")
                                                                           .size()];
                for (int i = 0; i < list.length; i++) {
                    list[i] = SystemPreislisten.hmPreisGruppen.get("Common")
                                                              .get(i);
                }
                String value = (String) JOptionPane.showInputDialog(null,
                        "Bitte wählen Sie die zutreffende Preisgruppe aus", "Title", JOptionPane.QUESTION_MESSAGE, null,
                        list, // selection values
                        SystemPreislisten.hmPreisGruppen.get("Common")
                                                        .get(0)); //
                String pg = Integer.toString(SystemPreislisten.hmPreisGruppen.get("Common")
                                                                             .indexOf(value)
                        + 1);
                ...
                /*
                 * String sql = "update kass_adr set kassen_nam1='"+name1+"', kassen_nam2='"
                 * +name2+"', strasse='"+ strasse+"', plz='"+plz+"', ort='"+ort+"', ik_kasse='"
                 * +ikkasse+"', ik_kostent='"+
                 * ikktraeger+"', ik_physika='"+ikdaten+"', ik_nutzer='"
                 * +iknutzer+"', ik_papier='"+ikpapier+"', "+
                ...
                /********** Datenannahmestelle **********/
                if (SqlInfo.holeEinzelFeld("select ik_kasse from kass_adr where ik_kasse='" + ikdaten + "' LIMIT 1")
                           .equals("")) {
                    JOptionPane.showMessageDialog(null, "Datenannahmestelle wird ebenfalls importiert");
                    Vector<Vector<String>> dvec = SqlInfo.holeFelder(
                            "select * from ktraeger where ikkasse='" + ikdaten + "' LIMIT 1");
                    if (dvec.size() <= 0) {
                        JOptionPane.showMessageDialog(null,
                                "Datenannahmestelle konnte nicht gefunden!\n\nDatenannahmestelle bitte manuell anlegen\n");
                    } else {
                        String sql = "insert into kass_adr set kassen_nam1='" + dvec.get(0)
                                                                                    .get(5)
                                + "', " + "kassen_nam2='" + dvec.get(0)
                                                                .get(6)
                                + "', " + "strasse='" + dvec.get(0)
                                                            .get(10)
                                + "', " + "plz='" + dvec.get(0)
                ...
                /********** Papierannahmestelle **********/
                if (SqlInfo.holeEinzelFeld("select ik_kasse from kass_adr where ik_kasse='" + ikpapier + "' LIMIT 1")
                           .equals("")) {
                    JOptionPane.showMessageDialog(null, "Papierannahmestelle wird ebenfalls importiert");
                    Vector<Vector<String>> dvec = SqlInfo.holeFelder(
                            "select * from ktraeger where ikkasse='" + ikpapier + "' LIMIT 1");
                    if (dvec.size() <= 0 || ikpapier.equals("")) {
                        JOptionPane.showMessageDialog(null,
                                "Papierannahmestelle konnte nicht gefunden!\n\nPapierannahmestelle bitte manuell anlegen\n");
                    } else {
                        String sql = "insert into kass_adr set kassen_nam1='" + dvec.get(0)
                                                                                    .get(5)
                                + "', " + "kassen_nam2='" + dvec.get(0)
