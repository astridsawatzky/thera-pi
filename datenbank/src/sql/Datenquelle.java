...
public class Datenquelle {
    ...
    public Datenquelle(IK ik) {
        File f = new File(environment.Path.Instance.getProghome() +"ini/" + ik.asString() + "/rehajava.ini");
        Ini ini;
        try {
            ini = new Ini(f );
            ini.load();
          ...
          String pw = Verschluesseln.getInstance().decrypt(ini.get(DATEN_BANK, "DBPasswort1"));
