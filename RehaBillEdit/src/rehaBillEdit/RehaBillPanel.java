...
class RehaBillPanel extends JXPanel implements ListSelectionListener, ActionListener, TableModelListener {
    ...
    private void doZeileLoeschen() {
        ...
        if (anfrage == JOptionPane.YES_OPTION) {
            ...
            String id = tabmod.getValueAt(tab.convertRowIndexToModel(row), tabmod.getColumnCount() - 1)
                              .toString()
                              .replace(".", "");
    ...
    private void rechneNeu(boolean inDBschreiben) {
        ...
        for (int i = 0; i < tabmod.getRowCount(); i++) {
            ...
            id = tabmod.getValueAt(i, tabmod.getColumnCount() - 1)
                       .toString()
                       .replace(".", "");
...
//			SqlInfo.sqlAusfuehren(cmd);
        ...
        try {
            ...
            if ("privat".equals(fuerwen) && heilmittel) {
                ...
                try {
                    ...
                    hmAdresse.put("<pri4>", ((String) tabmod.getValueAt(0, 3)).trim() + " "
                            + ((String) tabmod.getValueAt(0, 4)).trim());
                ...
                try {
                    ...
                    starteDokument(RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                            + RehaBillEdit.rhRechnungKasse);
                    System.out.println(RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                            + RehaBillEdit.rhRechnungKasse);
                    starteErsetzen();
                    aktuellePosition = 0;
                    startePositionen();
    ...
    private void doPrivatWiederholungsdruck(Vector<Vector<String>> preisvec) {
        ...
        try {
            System.out.println(RehaBillEdit.hmRechnungPrivat);
            starteDokument(RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                    + RehaBillEdit.hmAbrechnung.get("hmpriformular") + ".Kopie.ott");
            System.out.println(RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                    + RehaBillEdit.hmAbrechnung.get("hmpriformular") + ".Kopie.ott");
    ...
    private void doBGEWiederholungsdruck(Vector<Vector<String>> preisvec) {
        ...
        try {
            ...
            System.out.println(RehaBillEdit.hmRechnungPrivat);
            starteDokument(RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                    + RehaBillEdit.hmAbrechnung.get("hmbgeformular") + ".Kopie.ott");
            System.out.println(RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                    + RehaBillEdit.hmAbrechnung.get("hmbgeformular") + ".Kopie.ott");
            starteErsetzenBG();
            aktuellePosition = 0;
    ...
    private void doBegleitzetteldruck(String xreznr) {
        ...
        try {
            new BegleitzettelDrucken(rezanzahl, ik_kostent, name_kostent, hmAdresse, rnummerAlt.getText(),
                    RehaBillEdit.progHome + "vorlagen/" + RehaBillEdit.aktIK + "/"
                            + RehaBillEdit.hmAbrechnung.get("hmgkvbegleitzettel") + ".Kopie.ott");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
