...
public class LeistungTools {
    ...
    public static String[] getLeistung(String hmart, String hmposition, int preisgruppe) {
        ...
        for (int i = 0; i < 1;) {
            if (hmart.equals("KG")) {
                ...
                vec = hmPreise.get("Physio")
                              .get(preisgruppe);
                break;
            }
            if (hmart.equals("MA")) {
                // vec = ParameterLaden.vMAPreise;
                vec = hmPreise.get("Massage")
                              .get(preisgruppe);
                break;
            }
            if (hmart.equals("ER")) {
                // vec = ParameterLaden.vERPreise;
                vec = hmPreise.get("Ergo")
                              .get(preisgruppe);
                break;
            }
            if (hmart.equals("LO")) {
                // vec = ParameterLaden.vLOPreise;
                vec = hmPreise.get("Logo")
                              .get(preisgruppe);
                break;
            }
            if (hmart.equals("RH")) {
                // vec = ParameterLaden.vRHPreise;
                vec = hmPreise.get("Reha")
                              .get(preisgruppe);
                break;
            }
            if (hmart.equals("PO")) {
                // vec = ParameterLaden.vRHPreise;
                vec = hmPreise.get("Podo")
                              .get(preisgruppe);
                break;
            }
    ...
    public static String[] getLeistungRaw(String hmart, String hmposition, int preisgruppe) {
        ...
        if (vec.size() == 1) {
            return new String[] { String.valueOf(vec.get(0)
                                                    .get(0)),
                    String.valueOf(vec.get(0)
                                      .get(1)) };
        }
        return ret;
    }
