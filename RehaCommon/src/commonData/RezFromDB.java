...
public class RezFromDB {
    ...
    /**
     * initialisiert vecInArbeit mit allen Verordnungen zur uebergebenen Kasse, die
     * sich noch nicht im LZA befinden.
     *
     * @param kasse IK der zu pruefenden Kasse
     * @return true, wenn VOs existieren
     */
    private boolean initActiveVO(String kasse) {
        aktKasse = kasse;
        String sucheAktiveVO = "SELECT t1.rez_nr FROM verordn AS t1 LEFT JOIN kass_adr AS t2 ON t1.kid = t2.id where t2.ik_kasse="
                + aktKasse;
    ...
    public Vector<Vector<String>> getPendingVO(String kasse) {
        ...
        if (vecInArbeit.size() > 0) {
            ...
            for (Iterator<Vector<String>> it = vecFertige.iterator(); it.hasNext();) {
                Object curr = it.next();
