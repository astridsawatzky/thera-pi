...
public class RehaKassenbuchPanel extends JXPanel {
    ...
    private void doDelete() {
        ...
        if (item >= 0) {
            int anfrage = JOptionPane.showConfirmDialog(null,
                    "Wollen Sie das Kassenbuch --> " + kbvorhanden.getSelectedValue()
                                                                  .toString()
                            + " <-- wirklich löschen?",
                    "Achtung wichtige Benutzeranfrage", JOptionPane.YES_NO_OPTION);
            if (anfrage == JOptionPane.YES_OPTION) {
                String cmd = "drop table " + kbvorhanden.getSelectedValue()
                                                        .toString();
    ...
    private void doNew() {
        try {
            ...
            String tabelle = "kb_" + tfs[2].getText()
                                           .trim();
    ...
    private void doKBFuellen(String tabelle, String von, String bis) {
        // String cmd = "insert intoselect * into '"+tabelle+"' from kasse where datum
        // >= '"+von+"' and datum <= '"+bis+"'";
        String cmd2 = "CREATE TABLE " + tabelle + " SELECT * FROM kasse where datum >= '" + von + "' and datum <= '"
                + bis + "'";
