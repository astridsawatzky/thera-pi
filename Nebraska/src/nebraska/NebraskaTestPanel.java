...
public class NebraskaTestPanel extends JPanel implements ActionListener {
    ...
    public ImageIcon imgKey = new ImageIcon(
            (Constants.KEYSTORE_DIR + File.separator + "icons" + File.separator + "entry_pk.gif").replace("/",
                    File.separator));
    public ImageIcon imgCert = new ImageIcon(
            (Constants.KEYSTORE_DIR + File.separator + "icons" + File.separator + "certificate_node.gif").replace("/",
                    File.separator));
    ...
    private JPanel getTN1() {
        ...
        tabmodprax.setColumnIdentifiers(
                new String[] { "Alias", "Zert oder Key?", "CA-Root", "Gültig bis", "zurückgezogen" });
    ...
    private JPanel getButs1() {
        FormLayout lay = new FormLayout("5dlu,p,2dlu,p,2dlu,p,2dlu,p,5dlu", "5dlu,p,2dlu,p,5dlu");
        CellConstraints cc = new CellConstraints();
        PanelBuilder pb = new PanelBuilder(lay);
    ...
    public void doFuelleTabelle(boolean isRoot) throws Exception {
        // Hier werden die beiden Tabellen mit den Angaben zu enthaltenen Zertifikaten
        // gefüllt
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        try {
            ...
            if (cmd.equals("kgen1")) {
                ...
                KeyStore store = BCStatics2.loadStore(
                        Constants.KEYSTORE_DIR + File.separator + Constants.PRAXIS_OU_ALIAS.replace("IK", ""),
                        Constants.PRAXIS_KS_PW);
                PrivateKey privKey = (PrivateKey) store.getKey(Constants.PRAXIS_OU_ALIAS,
                        Constants.PRAXIS_KS_PW.toCharArray());
    ...
    public void doNextEncTest2() throws Exception {
        ...
        byte[] origBytes = FileStatics.BytesFromFile(
                new File(Constants.KEYSTORE_DIR + File.separator + "TSOL0021.org"));
        System.out.println("Länge des OriginalFiles in Byte: " + origBytes.length);
        byte[] result = createSignedMessage(origBytes, Constants.PRAXIS_OU_ALIAS, Constants.PRAXIS_OU_ALIAS,
                "IK109900019", "196205");
        FileStatics.BytesToFile(result, new File(Constants.KEYSTORE_DIR + File.separator + "TSOL0021"));
    }

    /******************************************************************/
    public byte[] createSignedMessage(byte[] dataParm, String pKeyAlias, String ourCertAlias, String theiCertAlias,
            String pw) {
        ...
        String keyStore = Constants.KEYSTORE_DIR + File.separator + Constants.PRAXIS_OU_ALIAS.replace("IK", "")
                + ".p12";
    ...
    public void doNextDecTest() throws Exception {
        ...
        BCStatics.providerTest();
        byte[] dataIn = FileStatics.BytesFromFile(
                new File(Constants.KEYSTORE_DIR + File.separator + "TSOL0021.org.encoded"));
        ...
        // String(envelopedData.getContentInfo().getEncoded()));
        System.out.println(envelopedData.getRecipientInfos()
                                        .getRecipients()
                                        .iterator());
        /*******************/
        ...
        Iterator it = envelopedData.getRecipientInfos()
                                   .getRecipients()
                                   .iterator();
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                recipient = envelopedData.getRecipientInfos()
                                         .get(iRID);
                ...
                while (en.hasMoreElements()) {
                    ...
                    if (aliases != null) {
                        ...
                        String serial = xcert.getSerialNumber()
                                             .toString(16);
                        int i = Integer.valueOf(sRID);
                        String hexRID = Integer.toHexString(i);
                        if (serial.equals(
                                hexRID) /* && xcert.getSubjectDN().toString().contains(Constants.PRAXIS_OU_ALIAS) */) {
        ...
        DERObject der = envelopedData.getContentInfo()
                                     .getDERObject();
        ...
        if (seq != null) {
            ...
            for (int i = 0; i < seqs; i++) {
                ...
                try {
                    ...
                    if (seq.getObjectAt(i)
                           .getDERObject()
                           .toASN1Object() instanceof org.bouncycastle.asn1.DERObjectIdentifier) {
                        org.bouncycastle.asn1.DERObjectIdentifier ident = (DERObjectIdentifier) seq.getObjectAt(i)
                                                                                                   .getDERObject()
                                                                                                   .toASN1Object();
                        ...
                        btag = (BERTaggedObject) seq.getObjectAt(i)
                                                    .getDERObject()
                                                    .toASN1Object();
                    }

                    // ASN1Sequence seq2 =
                    // (ASN1Sequence)((DERObject)seq.getObjectAt(i).getDERObject()).toASN1Object();
                    ...
                    for (int i2 = 0; i2 < bsize; i2++) {
                        ...
                        if ((bseq.getObjectAt(i2) instanceof org.bouncycastle.asn1.DERSet)) {
                            ...
                            for (int i4 = 0; i4 < bseq2.size(); i4++) {
                                ...
                                System.out.println("Klasse Sequenz2 " + i4 + " - " + bseq2.getObjectAt(i4)
                                                                                          .getClass());
                                if (bseq2.getObjectAt(i4) instanceof DERObjectIdentifier) {
                                    ...
                                    for (int i5 = 0; i5 < bseq3.size(); i5++) {
                                        ...
                                        System.out.println("Klasse von  i5 Nr. " + i5 + " = " + bseq3.getObjectAt(i5)
                                                                                                     .getClass());
                                        if (bseq3.getObjectAt(i5) instanceof DEROctetString) {
                                            ASN1OctetString doct = (ASN1OctetString) ((DEROctetString) bseq3.getObjectAt(
                                                    i5)).parser();
                                            // System.out.println("OktetString i5 = "+new String(doct.getOctets()));
                                            System.out.println("toHex i5 = " + NUtils.toHex(doct.getOctets()));
                                            KeyPair kp = BCStatics2.getBothFromPem(
                                                    Constants.KEYSTORE_DIR + File.separator + "540840108");
                                            ...
                                            RSAPrivateKeyStructure privStruct = new RSAPrivateKeyStructure(
                                                    (DERSequence) prikInfo.getPrivateKey());
                                            ...
                                            RSAPrivateCrtKeyParameters keySpec = new RSAPrivateCrtKeyParameters(modulo,
                                                    publicExponent, privateExponent, primo1, primo2, exponente1,
                                                    exponente2, coeficiente);
        ...
        // "+envelopedData.getContentInfo().getContentType());
        DERObjectIdentifier dident = envelopedData.getContentInfo()
                                                  .getContentType();
        // System.out.println("getDEREncoded = "+NUtils.toHex(dident.getDEREncoded()));

        ASN1EncodableVector avec = new ASN1EncodableVector();

        avec.add(der.getDERObject()
                    .toASN1Object());
    ...
    public byte[] doNextEncTest(String inFile, String kkAlias) throws Exception {
        ...
        // Für den Test eigenes Zertifikat, den PrivKey und das Zertifikat des
        // IKK-Bundesverbandes aus dem Keystore holen
        KeyStore store = BCStatics2.loadStore(
                Constants.KEYSTORE_DIR + File.separator + Constants.PRAXIS_OU_ALIAS.replace("IK", ""),
                Constants.PRAXIS_KS_PW);
        ...
        Iterator<?> its = signed.getSignerInfos()
                                .getSigners()
                                .iterator();
        if (its.hasNext()) {
        ...
        // gen.addKeyAgreementRecipient( CMSEnvelopedGenerator.DES_EDE3_CBC, privkey,
        // cert.getPublicKey(), certRecipient, CMSEnvelopedGenerator.DES_EDE3_WRAP,
        // "BC");
        gen.addKeyTransRecipient(praxiscert); // zum Test auf eigenes Zert verschlüsseln
        // gen.addKEKRecipient(key, kekID);

        // CMSProcessable data = new CMSProcessableFile(new
        // File(Constants.KEYSTORE_DIR+File.separator+"TSOL0021.org"));

        CMSEnvelopedData envelopedData = gen.generate(signed.getSignedContent(), CMSEnvelopedDataGenerator.DES_EDE3_CBC,
                "BC");
        // CMSEnvelopedData envelopedData =
        // gen.generate(data,CMSEnvelopedDataGenerator.DES_EDE3_CBC ,"BC");
        // envelopedData = new CMSEnvelopedData(envelopedData.getEncoded());
        ...
        /*
         * EncryptionKey ekey = EncryptionKey. Encryption enc =
         * security.getEncryption(); EncryptionOptions options =
         * enc.getDefaultEncryptionOptions(); options.setDataEncryptionKey(key);
        ...
        recID.setSubjectPublicKey(praxiscert.getPublicKey()
                                            .getEncoded());
        ...
        System.out.println("Recipients = " + envelopedData.getRecipientInfos()
                                                          .size());
        System.out.println("ContentType = " + envelopedData.getContentInfo()
                                                           .getContentType());
        System.out.println("EncryptionAlgOID = " + envelopedData.getEncryptionAlgOID());
        System.out.println("EnvelopedData = " + new String(envelopedData.getEncoded()));

        FileStatics.BytesToFile(envelopedData.getEncoded(),
                new File(Constants.KEYSTORE_DIR + File.separator + "TSOL0021"));
        // System.out.println(recipient.getKeyEncryptionAlgOID());
        /*
         * Iterator it = recipients.getRecipients().iterator(); while(it.hasNext()){
         * RecipientInformation inform = ((RecipientInformation)it.next()); String sRID
         * =
         * inform.getRID().getSerialNumber().toString();//((RecipientInformation)it.next
         * ()).getRID().getSerialNumber().toString(); RecipientId iRID =
         * inform.getRID();
         ...
         * xcert.getSerialNumber().toString(16); int i = Integer.valueOf(sRID); String
         * hexRID = Integer.toHexString(i); if(serial.equals(hexRID)){
         * System.out.println("Zertifikat gefunden = "+xcert.getSubjectDN()); } } } }
         */
    ...
    public void doNextEncTest3() throws Exception {
        ...
        CMSProcessable data = new CMSProcessableFile(
                new File(Constants.KEYSTORE_DIR + File.separator + "TSOL0021.org"));
        ...
        recID.setSubjectPublicKey(cert.getPublicKey()
                                      .getEncoded());
        ...
        // CMSProcessable dataToSign = new CMSProcessableFile(new
        // File(Constants.KEYSTORE_DIR+File.separator+"TSOL0021.org"));
        // CMSSignedData signed = sgen.generate(dataToSign,"BC");
        // signed = new CMSSignedData(dataToSign,signed.getEncoded());
        CMSSignedData signed = sgen.generate((CMSProcessable) envelopedData.getContentInfo()
                                                                           .getContent(),
                "BC");
    ...
    public void doCertAuswerten() throws Exception {
        ...
        KeyStore store = BCStatics2.loadStore(Constants.KEYSTORE_DIR + File.separator + "540840108",
                Constants.PRAXIS_KS_PW);
        X509Certificate cert = (X509Certificate) store.getCertificate(alias);
        String certInfo = "       Seriennummer = " + NUtils.toHex(cert.getSerialNumber()
                                                                      .toByteArray())
                + "\n";
        certInfo = certInfo + "    BasicConstaints = " + Integer.toString(cert.getBasicConstraints()) + "\n";
        certInfo = certInfo + "SignaturAlgorithmus = " + cert.getSigAlgName()
                                                             .toString()
                + "\n";
        certInfo = certInfo + "     SignaturAlgOID = " + cert.getSigAlgOID()
                                                             .toString()
                + "\n";
        certInfo = certInfo + "   Certificate-Type = " + cert.getType()
                                                             .toString()
                + "\n";
        certInfo = certInfo + "Certificate-Version = " + Integer.toString(cert.getVersion()) + "\n";
        certInfo = certInfo + "         Subject-DN = " + cert.getSubjectDN()
                                                             .toString()
                + "\n";
        ...
        certInfo = certInfo + " MD5-Fingerpr. Cert = "
                + BCStatics2.macheHexDump(BCStatics2.getMD5fromByte(cert.getEncoded()), 20, " ") + "\n";
        certInfo = certInfo
                + "MD5-Fingerpr.PubKey = " + BCStatics2.macheHexDump(BCStatics2.getMD5fromByte(sub.getPublicKeyData()
                                                                                                  .getBytes()),
                        20, " ")
                + "\n";
        ...
        while (en.hasMoreElements()) {
            ...
            if (aliases != null) {
                if (store.isCertificateEntry(aliases)) {
                    ...
                    // System.out.println("KeyEntry = "+store.getEntry(aliases,new
                    // KeyStore.PasswordProtection(Constants.PRAXIS_KS_PW.toCharArray()) ));
                    // certVec.add((X509Certificate) store.getCertificate(aliases));
                    // certAlias.add(aliases);
    ...
    private Vector macheTabellenZeile(boolean isKeyEntry, String alias, X509Certificate cert) throws Exception {
        ...
        // BasicConstraints bc =
        // BasicConstraints.getInstance(cert.getBasicConstraints());
        zeile.add("?");
    ...
    private void doCertTest() throws Exception {
        KeyStore store = BCStatics2.loadStore(Constants.KEYSTORE_DIR + File.separator + "540840108",
                Constants.PRAXIS_KS_PW);
        /*
         * Certificate[] chain = store.getCertificateChain("IK540840108"); for(int i =
         * 0; i < chain.length;i++){
        ...
        for (int i = 0; i < chainAnnahme.length; i++) {
            alias = BCStatics3.extrahiereAlias(chainAnnahme[i].getSubjectDN()
                                                              .toString());
            if (alias == null) {
                alias = chainAnnahme[i].getSubjectDN()
                                       .toString();
            }
        ...
        // store.setKeyEntry(vecprax.get(0),
        // kpprax.getPrivate(),praxisPassw.toCharArray(),chainGesamt );
        ...
        for (int i = 0; i < lcerts.size(); i++) {
            alias = BCStatics3.extrahiereAlias(((X509Certificate) lcerts.get(i)).getSubjectDN()
                                                                                .toString());
            if (alias == null) {
                alias = ((X509Certificate) lcerts.get(i)).getSubjectDN()
                                                         .toString();
            }
        ...
        FileStatics.BytesToFile(bout.toByteArray(),
                new File(Constants.KEYSTORE_DIR + File.separator + "540840108.p12"));
        bout.close();
        // System.out.println(certPath);
        // byte[] bp = certPath.getEncoded();
        // FileStatics.BytesToFile(bp, new
        // File(Constants.KEYSTORE_DIR+File.separator+"540840108.p12"));

        /*
         * String alias = ""; for(int i = 0; i < chainAnnahme.length;i++){ alias =
         * BCStatics3.extrahiereAlias(chainAnnahme[i].getSubjectDN().toString());
         * store.setEntry(alias, new KeyStore.TrustedCertificateEntry(chainAnnahme[i]),
         * null); } ByteArrayOutputStream bOut = new ByteArrayOutputStream();
         * store.store(bOut, Constants.PRAXIS_KS_PW.toCharArray());
         * FileStatics.BytesToFile(bOut.toByteArray(), new
         * File(Constants.KEYSTORE_DIR+File.separator+"540840108.p12")); bOut.close();
         */
    ...
    private void doMacheKeyStoreNeu() throws Exception {
        ...
        /*
         * KeyPair kp =
         * BCStatics2.getBothFromPem(Constants.KEYSTORE_DIR+File.separator+"540840108");
         * KeyStore secretKeyStore =
         * BCStatics2.loadStore(Constants.KEYSTORE_DIR+File.separator+"540840108_key",
         * Constants.PRAXIS_KS_PW); X509Certificate[] chain = new X509Certificate[1];
         * chain[0] = BCStatics2.generateSelfSignedV3Certificate(kp, vecprax, vecca);
         * secretKeyStore.setKeyEntry("IK540840108",
         * kp.getPrivate(),"196205".toCharArray(),chain ); try{
        ...
        KeyStore secretKeyStore = BCStatics2.loadStore(Constants.KEYSTORE_DIR + File.separator + "540840108_key",
                Constants.PRAXIS_KS_PW);
        KeyStore keyStore = BCStatics2.loadStore(Constants.KEYSTORE_DIR + File.separator + "540840108",
                Constants.PRAXIS_KS_PW);
        KeyStore trustStore = BCStatics2.loadStore(Constants.KEYSTORE_DIR + File.separator + "540840108_key",
                Constants.PRAXIS_KS_PW);
        ...
        // boolean geklappt = BCStatics2.installReply(keyStore, trustStore, "196205",
        // "C=DE,O=Datenaustausch im Gesundheits- und Sozialwesen", in, true, true,
        // kp.getPrivate());
        boolean geklappt = BCStatics2.installReply(keyStore, trustStore, "196205", "IK540840108", in, true, true,
                kp.getPrivate());
        ...
        /*
         * Object[] annahmeRet = BCStatics3.readAnnahme(true,null); int anzahl =
         * (Integer)annahmeRet[0];
         * System.out.println("Es befinden sich "+anzahl+" Zertifikate in der Datei");
         * if(anzahl <= 0){ return; } X509Certificate[] chain = new
         * X509Certificate[anzahl]; annahmeRet = BCStatics3.readAnnahme(false,chain);
        ...
        for (int i = 0; i < chain.length; i++) {
            alias = BCStatics3.extrahiereAlias(chain[i].getSubjectDN()
                                                       .toString());
            try {
        ...
        // Certificate[] chainCerts = (Certificate[])
        // keyStore.getCertificateChain("C=DE,O=ITSG TrustCenter fuer sonstige
        // Leistungserbringer");
        for (int i = 0; i < chainCerts.length; i++) {
            System.out.println(((X509Certificate) chainCerts[i]).getSubjectDN() + " - "
                    + ((X509Certificate) chainCerts[i]).getIssuerDN() + " - "
                    + ((X509Certificate) chainCerts[i]).getBasicConstraints());
        }
    ...
    private void doCertAntrag() throws Exception {
        ...
        // FileInputStream(Constants.KEYSTORE_DIR+File.separator+"40091472.p10"));
        /*
         * ASN1InputStream ain = new ASN1InputStream(new
         * FileInputStream(Constants.KEYSTORE_DIR+File.separator+"54084010.p10"));
         * DERObject derob = ain.readObject(); PKCS10CertificationRequest csr = new
         * PKCS10CertificationRequest( derob.getDEREncoded() );
         ...
         * getPublicKey());
         * System.out.println("SubjectDN: "+BCStatics2.getMD5fromByte(csrInfo.
         * getSubjectPublicKeyInfo().getPublicKeyData().getBytes()));
         */
        String outFile = null;
        PdfReader reader = new PdfReader(
                Constants.KEYSTORE_DIR + File.separator + "vorlagen" + File.separator + "Zertifizierungsantrag.pdf");
        outFile = Constants.KEYSTORE_DIR + File.separator + "vorlagen" + File.separator + "Zertifizierungsantrag"
                + DatFunk.sHeute() + ".pdf";
    ...
    private void saveTestCase() {
        ...
        // In der Entwicklungsversion werden verschiedene Items nicht gespeichert z.b.
        // CE oder PRAXIS_O
        inif.setStringProperty("Praxis", "PRAXIS_OU_ALIAS", tn1[0].getText()
                                                                  .trim(),
                null);
        inif.setStringProperty("Praxis", "PRAXIS_OU_FIRMA", tn1[1].getText()
                                                                  .trim(),
                null);
        inif.setStringProperty("Praxis", "PRAXIS_CN", tn1[2].getText()
                                                            .trim(),
                null);
        inif.setStringProperty("Praxis", "PRAXIS_KS_PW", tn1[3].getText()
                                                               .trim(),
                null);
        inif.setStringProperty("TestCA", "TEST_CA_OU_ALIAS", tn2[0].getText()
                                                                   .trim(),
                null);
        inif.setStringProperty("TestCA", "TEST_CA_OU_FIRMA", tn2[1].getText()
                                                                   .trim(),
                null);
        inif.setStringProperty("TestCA", "TEST_CA_O", tn2[3].getText()
                                                            .trim(),
                null);
        inif.setStringProperty("TestCA", "TEST_CA_CN", tn2[2].getText()
                                                             .trim(),
                null);
        inif.setStringProperty("TestCA", "TEST_CA_KS_PW", tn2[4].getText()
                                                                .trim(),
                null);
        inif.save();
        JOptionPane.showMessageDialog(null,
                "Die aktuellen Angaben zu Praxis und CA wurden gesichert in Datei:\n" + Constants.INI_FILE);
    ...
    public void doSHA1Test() throws Exception {
        ...
        BCStatics2.deleteCertFromStore(null, Constants.KEYSTORE_DIR + File.separator + "540840108",
                Constants.PRAXIS_KS_PW);
        /*
         ...
         * System.out.println("SHA1-.p7b-File      "+BCStatics2.getSHA1fromByte(pairDb.
         * getEncoded()));
         * System.out.println("SHA1-PEM-File       "+BCStatics2.getSHA1fromByte(pairPem.
         * getPublic().getEncoded()));
         * System.out.println("SHA1-Keystore-File  "+BCStatics2.getSHA1fromByte(pairSt.
         * getEncoded())); System.out.
         * println("**************** SHA1 Fingerprint Zertifikat im KeyStore ******************"
    ...
    public void doAnnahmeReadAndStore() throws Exception {
        ...
        for (int i = 0; i < anzahl; i++) {
            ...
            // BCStatics2.importCertIntoStore(chain[i], keystoreDir+File.separator,
            // praxisPassw, vecprax.get(0).replace("IK", "")+"_key");
        }
        ...
        for (int i = 0; i < anzahl; i++) {
            // System.out.println("Zertifikat Nr. "+i+" =
            // "+lcert.get(i).getBasicConstraints());
    ...
    public String getAliasFromCert(X509Certificate cert) {
        String alias = cert.getSubjectDN()
                           .toString();
        return alias;
    }
    ...
    public void doCertCreate() throws Exception {
        KeyPair kpprax = null;
        String datei = keystoreDir + File.separator + vecprax.get(0)
                                                             .replace("IK", "");
        File f = new File(datei + ".prv");
        if (f.exists()) {
            KeyPair kp = BCStatics2.getBothFromPem(datei);
            kpprax = kp;
            System.out.println("SHA1 von PrivateKey (aus Pem-File) = " + BCStatics2.getSHA1fromByte(kpprax.getPrivate()
                                                                                                          .getEncoded()));
            System.out.println("SHA1 von PublicKey (aus Pem-File) " + BCStatics2.getSHA1fromByte(kpprax.getPublic()
                                                                                                       .getEncoded()));
        } else {
        ...
        // store.setKeyEntry("secret", kpprax.getPrivate(),praxisPassw.toCharArray(),
        // chain);

        BCStatics2.saveStore(store, praxisPassw, vecprax.get(0)
                                                        .replace("IK", ""));
        // BCStatics2.certToFile(cert, cert.getEncoded(), keystoreDir+ File.separator
        // +"test");

        // System.out.println(cert);
        // FileStatics.BytesToFile(cert.getEncoded(), new File(keystoreDir+
        // File.separator +"test.p7b"));

    }

    public void doEncode() throws Exception {
        BCStatics2.verschluesseln(vecprax.get(0), keystoreDir + File.separator + vecprax.get(0)
                                                                                        .replace("IK", ""),
                praxisPassw);
    }

    public void doReadAndManageReply() throws Exception {
        BCStatics2.readCertReply(keystoreDir, vecprax.get(0)
                                                     .replace("IK", ""),
                praxisPassw);
    }
    ...
    public void doGenerateRequest() throws Exception {
        ...
        for (int i = 0; i < aseq.size(); i++) {
            ...
            if (aseq.getObjectAt(i) instanceof SubjectPublicKeyInfo) {
                ...
                System.out.println(
                        "SHA1-Hash aus dem PubKey des Requests = " + BCStatics2.getSHA1fromByte(spub.getPublicKeyData()
                                                                                                    .getEncoded()));
            }
        }
        System.out.println("Origianl Public Key des PEM-Files = " + kpprax.getPublic());
        System.out.println("SHA-1 Fingerprint des Zerifikates = "
                + BCStatics2.getSHA1fromByte(FileStatics
                                                        .BytesFromFile(new File(keystoreDir
                                                                + File.separator + vecprax.get(0)
                                                                                          .replace("IK", "")
                                                                + ".p7b"))));
        ...
        System.out.println("SHA-1-Fingerprint PublicKey  =" + BCStatics2.getSHA1fromByte(kpprax.getPublic()
                                                                                               .getEncoded()));
        ...
        Nebraska.hmZertifikat.put("<Algorithm>", kpprax.getPublic()
                                                       .getAlgorithm());

        String sha1 = BCStatics2.getSHA1fromByte(spub.getPublicKeyData()
                                                     .getBytes());
        ...
        java.security.interfaces.RSAPublicKey pub = (java.security.interfaces.RSAPublicKey) kpprax.getPublic();
        ...
        OOorgTools.starteStandardFormular(
                keystoreDir + File.separator + "vorlagen" + File.separator + "ZertBegleitzettel.ott", null);
    ...
    private boolean doKeystore(String passw, boolean isRoot) throws Exception {
        ...
        String keystore = "";
        if (isRoot) {
            String testdatei = keystoreDir + File.separator + vecca.get(0)
                                                                   .replace("IK", "");
            ...
            String testdatei = keystoreDir + File.separator + vecprax.get(0)
                                                                     .replace("IK", "");
            File f = new File(testdatei + ".prv");
            if (f.exists()) {
                KeyPair kp = BCStatics2.getBothFromPem(testdatei);
            ...
            datei = tn1[0].getText()
                          .trim()
                          .replace("IK", "");
            keystore = keystoreDir + File.separator + tn1[0].getText()
                                                            .trim()
                                                            .replace("IK", "")
                    + ".p12";
            if (tn1[0].getText()
        ...
        if (!f.exists()) {
            ...
            BCStatics2.createKeyStore(datei, passw, isRoot, vecprax, vecca, (isRoot ? kpca : kpprax));
            return true;
        } else {
            Nebraska.jf.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
