...
public class BCStatics2 {
    ...
    public static void createKeyStore(String name, String passw, boolean isroot, Vector<String> praxvec,
            Vector<String> cavec, KeyPair kp) throws Exception {
        providerTest();
        KeyStore store;
        ...
        if (isroot) {
            ...
            chain[0] = cert;
            // store.setKeyEntry(alias,keypair.getPrivate(),passw.toCharArray(),(java.security.cert.Certificate[])
            // chain);
        }
        ...
        FileStatics.BytesToFile(bOut.toByteArray(),
                new File(NebraskaTestPanel.keystoreDir + File.separator + name + ".p12"));
        bOut.close();
        System.out.println("\n*********************Zertifikat Beginn************************");
    ...
    public static X509Certificate generateRootCert(KeyPair pair, Vector<String> vecca)
        ...
        certGen.setSubjectDN(new X500Principal("CN=" + vecca.get(2) + ",OU=" + vecca.get(0) + "," + "OU=" + vecca.get(1)
                + ", O=" + vecca.get(3) + ",C=DE"));
    ...
    public static X509Certificate generateV3Certificate(KeyPair pair, Vector<String> praxvec, Vector<String> cavec)
        ...
        certGen.setNotBefore(new Date(BCStatics2.certifikatsDatum(DatFunk.sHeute(), 0)));
        certGen.setNotAfter(new Date(BCStatics2.certifikatsDatum(DatFunk.sHeute(), 3)));
        certGen.setSubjectDN(new X500Principal("CN=" + praxvec.get(2) + ",OU=" + praxvec.get(0) + "," + "OU="
                + praxvec.get(1) + ", O=" + cavec.get(3) + ",C=DE"));
        certGen.setPublicKey(pair.getPublic());
        certGen.setSignatureAlgorithm("SHA1WithRSAEncryption");
        /*
         * certGen.addExtension(X509Extensions.BasicConstraints,true, new
         * BasicConstraints(true)); certGen.addExtension(X509Extensions.KeyUsage,true,
         * new KeyUsage(KeyUsage.digitalSignature | KeyUsage.keyEncipherment));
    ...
    /*********************************/
    public static PKCS10CertificationRequest generateRequest(KeyPair pair, Vector<String> praxvec, Vector<String> cavec)
            throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, SignatureException {
        providerTest();
        System.out.println("Pbulic Key SHA-1 Hash = " + getSHA1fromByte(pair.getPublic()
                                                                            .getEncoded()));
        System.out.println("  Public Key MD5 Hash = " + getMD5fromByte(pair.getPublic()
                                                                           .getEncoded()));

        return new PKCS10CertificationRequest(
                "SHA1withRSA", new X500Principal("CN=" + praxvec.get(2) + "," + "OU=" + praxvec.get(0) + ",OU="
                        + praxvec.get(1) + "," + "O=" + cavec.get(3) + ",C=DE"),
                pair.getPublic(), null, pair.getPrivate());
    }

    /*********************************/
    public static X509Certificate[] makeReply(String csrPfad, String keystorefile, String alias, String passw)
            throws Exception {
        providerTest();
        ...
        PKCS10CertificationRequest csr = new PKCS10CertificationRequest(derob.getDEREncoded());
         ...
         * SubjectKeyIdentifierStructure(csr.getPublicKey("BC")));
         * certGen.addExtension(X509Extensions.BasicConstraints,false, new
         * BasicConstraints(true)); certGen.addExtension(X509Extensions.KeyUsage,true,
         * new KeyUsage(KeyUsage.digitalSignature | KeyUsage.keyEncipherment));
         * certGen.addExtension(X509Extensions.ExtendedKeyUsage,true, new
         * ExtendedKeyUsage(KeyPurposeId.id_kp_serverAuth)); ASN1Set attributes =
         * csr.getCertificationRequestInfo().getAttributes();
         * System.out.println("Attribute ist "+attributes); if(attributes != null){
         * for(int i = 0; i != attributes.size();i++){ Attribute attr =
         * Attribute.getInstance(attributes.getObjectAt(i));
         * if(attr.getAttrType().equals(PKCSObjectIdentifiers.pkcs_9_at_extensionRequest
         * )){ X509Extensions extensions =
         * X509Extensions.getInstance(attr.getAttrValues().getObjectAt(0)); Enumeration
         * e = extensions.oids(); while(e.hasMoreElements()){ DERObjectIdentifier oid =
         * (DERObjectIdentifier)e.nextElement(); X509Extension ext =
         * extensions.getExtension(oid); certGen.addExtension(oid, ext.isCritical()
    ...
    /***************************************************************************/
    public static int readMultipleAnnahme(String keystoreDir, String pw, String xalias, boolean checkonly,
    ...
    public static void importCertIntoStore(X509Certificate cert, String keystore, String pw, String xalias)
            throws Exception {
        KeyStore store;
        String alias;
        store = KeyStore.getInstance("BCPKCS12", "BC");
        ...
        if (!store.containsAlias(alias.trim())) {
            store.setCertificateEntry(alias, cert);
            System.out.println("Zertifikat von " + alias + " der Datenbank " + keystore + File.separator + xalias
                    + ".p12" + " hinzugefügt");
            ByteArrayOutputStream bOut = new ByteArrayOutputStream();
            ...
            int frage = JOptionPane.showConfirmDialog(null,
                    "Zertifikat ist bereits in der Datenbank enthalten.\nTrotzdem importieren?", "Achtung!",
    ...
    private static void doCertPath(Vector<X509Certificate> certVec, String keystoreDir, String keystoreFile,
        ...
        for (int i = 0; i < chain2.size(); i++) {
            String alias = BCStatics3.extrahiereAlias(((X509Certificate) chain2.get(i)).getSubjectDN()
                                                                                       .toString())
                                     .trim();
            System.out.println("Alias = " + alias);
            // BCStatics2.importCertIntoStore((X509Certificate) chain2.get(i),keystoreDir +
            // File.separator +keystoreFile,keystorePassword,"");
        ...
        endConstraints.setIssuer(chain3[2].getIssuerX500Principal()
                                          .getEncoded());
        ...
        PKIXCertPathBuilderResult result = (PKIXCertPathBuilderResult) builder.build(buildParms);

        CertPath path = result.getCertPath();
        Iterator it = path.getCertificates()
                          .iterator();
        while (it.hasNext()) {
         ...
         * ByteArrayOutputStream bOut = new ByteArrayOutputStream();
         * store.store(bOut,"196205".toCharArray());
         * FileStatics.BytesToFile(bOut.toByteArray(),new File(Constants.KEYSTORE_DIR +
         * File.separator +"teststore"+".p12")); bOut.close();
         */
        // KeyStore keyStore =
        // BCStatics2.loadStore(Constants.KEYSTORE_DIR+File.separator+keystoreFile,
        // keystorePassword);
    ...
    public static void certToFile(X509Certificate x509Cert, byte[] b, String name) throws IOException {
        ...
        fos.close();
        /*
         * f = new File(name+".pem"); fos = new FileOutputStream(f);
         * fos.write(x509Cert.toString().getBytes()); fos.flush(); fos.close();
         */
    ...
    public static X509Certificate generateSelfSignedV3Certificate(KeyPair pair, Vector<String> praxvec,
            Vector<String> cavec) throws Exception {
        providerTest();
        X509V3CertificateGenerator certGen = new X509V3CertificateGenerator();
        certGen.setSerialNumber(BigInteger.valueOf(1));
        certGen.setIssuerDN(new X500Principal("O=" + cavec.get(3) + ",C=DE"));
         ...
         * time = new Time(d.getTime());
         * certGen.addExtension(X509Extensions.BasicConstraints, true, new
         * BasicConstraints(false)); certGen.addExtension(X509Extensions.KeyUsage, true,
         * new KeyUsage(KeyUsage.digitalSignature | KeyUsage.keyEncipherment));
         * certGen.addExtension(X509Extensions.ExtendedKeyUsage, true, new
         * ExtendedKeyUsage( KeyPurposeId.id_kp_serverAuth));
         */
    ...
    static X509Extensions getX509ExtensionsFromCsr(PKCS10CertificationRequest certificateSigningRequest)
            throws Exception {
            ...
            if (derEncodable instanceof DERSequence) {
                ...
                if (attribute.getAttrType()
                    ...
                    if (attributeValues.size() >= 1) {
                        certificateRequestExtensions = new X509Extensions(
                                (ASN1Sequence) attributeValues.getObjectAt(0));
                        break;
                    }
                }
            }
    ...
    public static void verschluesseln(String alias, String keystoreFile, String pw) throws Exception {
        ...
        if (store.containsAlias(alias)) {
            ...
            for (int i = 0; i < 40; i += 2) {
                keyBytes[i / 2] = sha.substring(i, i + 2)
                                     .getBytes()[0];
            }
    ...
    public static void storePrivateToPem(KeyPair keypair, String name) throws Exception {
        ...
        fWriter.close();
        /*
         * fWriter = new FileWriter(new File(NebraskaTestPanel.keystoreDir+
         * File.separator +name+".pub")); pemWriter = new PEMWriter(fWriter, "BC");
         * pemWriter.writeObject(keypair.getPublic()); pemWriter.close();
         * fWriter.close(); fWriter = new FileWriter(new
         * File(NebraskaTestPanel.keystoreDir+ File.separator +name+".sls")); pemWriter
         * = new PEMWriter(fWriter, "BC"); pemWriter.writeObject(keypair); .close();
         * fWriter.close(); FileReader fReader = new FileReader(new
         * File(NebraskaTestPanel.keystoreDir+ File.separator +name+".prv")); PEMReader
         * pReader = new PEMReader(fReader); KeyPair pKey =
         * (KeyPair)pReader.readObject();
         * if(pKey.getPrivate().equals(keypair.getPrivate())){
         * System.out.println("Private Schl�ssel sind identisch"); }else{
    ...
    public static boolean installReply(KeyStore keyStore, KeyStore trustStore, String keyPassword, String alias,
            InputStream inputStream, boolean trustCACerts, boolean validateRoot, PrivateKey privkey) throws Exception {
        ...
        if (certs.size() == 1) {
            ...
            System.out.println("Beginn Chain of certs******************************" + keyStore + "-" + trustStore + "-"
                    + alias + "-" + certificate + "-" + certs);
    ...
    private static List<X509Certificate> establishCertChain(KeyStore keyStore, KeyStore trustStore,
            X509Certificate certificate, X509Certificate certReply, boolean trustCACerts) throws Exception {
    ...
    /**
     * Builds the certificate chain of the specified certificate based on the known
     * list of certificates that were issued by their respective Principals. Returns
     * true if the entire chain of all certificates was successfully built.
     *
     * @param certificate certificate to build its chain.
     * @param answer      the certificate chain for the corresponding certificate.
     * @param knownCerts  list of known certificates grouped by their issues (i.e.
     *                    Principals).
    ...
    private static boolean buildChain(X509Certificate certificate, LinkedList<X509Certificate> answer,
        ...
        // Check if the certificate is a root certificate (i.e. was issued by the same
        // Principal that
    ...
    private static List<X509Certificate> validateReply(KeyStore keyStore, KeyStore trustStore, String alias,
        ...
        for (i = 0; i < replyCerts.size(); i++) {
            if (userPubKey.equals(replyCerts.get(i)
                                            .getPublicKey())) {
                break;
            }
        }
        ...
        Principal issuer = replyCerts.get(0)
                                     .getIssuerDN();
        ...
        for (i = 0; i < replyCerts.size() - 1; i++) {
            PublicKey pubKey = replyCerts.get(i + 1)
                                         .getPublicKey();
            try {
                replyCerts.get(i)
                          .verify(pubKey);
        ...
        if (!foundInKeyStore && !foundInCAStore) {
            ...
            if (!verified) {
                ...
                if (!topCert.getSubjectDN()
                            .equals(topCert.getIssuerDN())) {
