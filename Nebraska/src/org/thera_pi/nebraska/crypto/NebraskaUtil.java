...
public class NebraskaUtil {
    ...
    /**
     * This is the set of charactes allowes by ITSG Trust Center. It is a subset of
     * PrintableString.
     */
    private static final String allowedCharacters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 /.()-";

    /**
     * Create Subject DN string from variable parts and constant part. X500Principal
     * is used to normalize the string.
     * 
    ...
    public static String getSubjectDN(String IK, String institutionName, String personName) {
        NebraskaPrincipal principal = new NebraskaPrincipal(NebraskaConstants.X500_PRINCIPAL_COUNTRY,
                NebraskaConstants.X500_PRINCIPAL_ORGANIZATION, institutionName, IK, personName);
        return principal.getDistinguishedName();
    }

    /**
     * Create a String that contains only characters of the PrintableString subset
     * allowed by ITSG Trust Center.
     * 
    ...
    /**
     * Get alias for certificate (optionally with private key) for specified
     * institution ID. Currently the alias is identical to the institution ID.
     * 
    ...
    /**
     * Get alias for self-signed key pair for specified institution ID. Currently
     * the alias the institution ID prepended with new.
     * 
     * @param institutionID the institution ID (IK)
     * @return the alias
     */
    ...
    /**
     * Generate a start date for the validity of a certificate. Use the beginning of
     * the specified day as start date.
     * 
    ...
    /**
     * Generate an end date for the validity of a certificate. The specified day
     * defines the start date. The end date will be calculated from the start date
     * with the default validity duration.
     * 
     * @param date specifies the beginning day of the certificate's validity
    ...
    /**
     * Generate a start or end date for the validity of a certificate. Use the
     * beginning of the specified day as start date or for an end date add the
     * certificate duration in years and subtract one millisecond to get the end of
     * the day before.
     * 
    ...
    /**
     * Convert a byte array to a hexadecimal string representation and use a
     * separator between the bytes for better readability. Intended to be used for
     * MD5 fingerprints.
     * 
