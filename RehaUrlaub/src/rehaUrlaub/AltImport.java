...
public class AltImport extends JXPanel {
        ...
        al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (cmd.equals("urlaubtabelle")) {
                    urlaubtabelle = "urlaub" + jahrecombo.getSelectedItem()
                                                         .toString()
                                                         .trim();
    ...
    private void doImportWahl() {
        ...
        if ((!ximportdatei.contains(usercombo.getSelectedItem()
                                             .toString()))
                || (!ximportdatei.contains(jahrecombo.getSelectedItem()
                                                     .toString()))) {
            JOptionPane.showMessageDialog(null,
                    "Der eingestellte Kalenderbenutzer und die ausgwählte Importdatei passen nicht zusammen");
            importdatei.setText("");
            return;
        }
    ...
    private void holeImportFelder() {
        ...
        for (int i = 1; i < 22; i++) {
            vecimportfelder.add(String.valueOf(dummy.get(i)
                                                    .get(0)));
        }
    ...
    private void doStarten() {
        ...
        if (vectabellen.indexOf(urlaubtabelle) < 0) {
            JOptionPane.showMessageDialog(null,
                    "Die Tabelle " + urlaubtabelle + " existiert nicht.\nBitte im ersten Karteireiter anlegen");
            return;
        }
        vecKollegenUrlaub.clear();
        vecKollegenUrlaub.trimToSize();
        String jahr = this.jahrecombo.getSelectedItem()
                                     .toString()
                                     .trim();
        String kalzeile = usercombo.getValueAt(1)
                                   .toString();
        String cmd = "select id from " + urlaubtabelle + " where jahr='" + jahr + "' and kal_zeile='" + kalzeile
        ...
        try {
            dbfreader = new DBFReader(importdatei.getText()
                                                 .trim());
            int durchlauf = 0;
            while (dbfreader.hasNextRecord()) {
                ...
                buf.append(" where id='" + vecKollegenUrlaub.get(durchlauf)
                                                            .get(0)
                        + "' LIMIT 1");
